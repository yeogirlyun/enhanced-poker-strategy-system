...
class BotSessionStateMachine(FlexiblePokerStateMachine):
    ...
    def start_session(self) -> bool:
        ...
        try:
            # For Hands Review, automatically set up HandModelDecisionEngine if we have preloaded data
            if (hasattr(self, 'preloaded_hand_data') and 
                self.preloaded_hand_data and 
                'hand_model' in self.preloaded_hand_data and
                hasattr(self.decision_engine, '__class__') and 
                'Fallback' in self.decision_engine.__class__.__name__):
                
                print("🔄 HANDS_REVIEW: Auto-switching to HandModelDecisionEngine")
                try:
                    from .hand_model_decision_engine import HandModelDecisionEngine
                    hand_model = self.preloaded_hand_data['hand_model']
                    self.decision_engine = HandModelDecisionEngine(hand_model)
                    print(f"✅ HANDS_REVIEW: Switched to HandModelDecisionEngine")
                except Exception as e:
                    print(f"❌ HANDS_REVIEW: Failed to switch to HandModelDecisionEngine: {e}")
...
class GTOBotSession(BotSessionStateMachine):
    ...
    def __init__(self, config: GameConfig, mode: str = "live"):
        ...
        self._hand_count = 0
        self._premium_test_mode = False  # default off
    ...
    def start_hand(self):
        ...
        # Only force premium hands if test mode is enabled
        if self._premium_test_mode:
...
class HandsReviewBotSession(BotSessionStateMachine):
    ...
    def _start_hand_from_hand_model(self, hand_model: Hand):
        ...
        setattr(self, "skip_shuffle", True)
        
        # === BEGIN: Deterministic deck for Hands Review ===
        # Build the preloaded board in deal order: flop(3) -> turn(1) -> river(1)
        try:
            from .hand_model import Street  # use your enum

            def _cards_from(street):
                # Handle both list[str] and list[CardLike]
                st = hand_model.streets.get(street)
                if not st or not getattr(st, "board", None):
                    return []
                out = []
                for c in st.board:
                    # normalize to 'AS', 'Td', etc. (adapt if your type differs)
                    out.append(str(getattr(c, "code", getattr(c, "card", c))))
                return out

            flop = _cards_from(Street.FLOP)[:3]
            turn = _cards_from(Street.TURN)[3:4] if len(_cards_from(Street.TURN)) >= 4 else _cards_from(Street.TURN)[-1:]
            river = _cards_from(Street.RIVER)[4:5] if len(_cards_from(Street.RIVER)) >= 5 else _cards_from(Street.RIVER)[-1:]
            preloaded_board_sequence = flop + turn + river

            # Collect used cards (all hole cards + the board) so they don't reappear
            used = set(preloaded_board_sequence)
            for p in self.game_state.players:
                for c in getattr(p, "cards", []):
                    used.add(str(c))

            # Create a full deck using the base helper, then remove used and prepend the board
            full_deck = self._create_deck()  # method of FlexiblePokerStateMachine
            full_deck = [str(c) for c in full_deck]
            remaining = [c for c in full_deck if c not in used]
            self.game_state.deck = preloaded_board_sequence + remaining

            # Quick sanity checks
            assert self.game_state.deck[:3] == flop,  "Flop mismatch at deck head"
            assert self.game_state.deck[3:4] == turn, "Turn mismatch after flop"
            assert self.game_state.deck[4:5] == river, "River mismatch after turn"

            print(f"🃏 HANDS_REVIEW: Deterministic deck installed: {self.game_state.deck[:5]}")
        except Exception as e:

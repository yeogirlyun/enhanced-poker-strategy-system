#!/usr/bin/env python3
"""
Simple test to isolate tier selection issue
"""
...
def simple_test():
    """Simple test to isolate the tier selection issue."""
    root = tk.Tk()
    root.title("Simple Tier Selection Test")
    ...
    def on_tier_select(selected_tiers):
        ...
        print(f"=== END TIER SELECTION CALLBACK ===")
    ...
    def test_silver_only():
        ...
        tier_panel.tier_listbox.selection_set(3)  # Silver tier (index 3)
    ...
    def test_elite_only():
        ...
        tier_panel.tier_listbox.selection_clear(0, tk.END)
        tier_panel.tier_listbox.selection_set(0)  # Elite tier (index 0)
        tier_panel._on_listbox_select(None)
    ...
    instructions = ttk.Label(main_frame, 
                           text="Instructions:\n1. Click 'Test Silver Only' to select only Silver tier\n2. Check console output for debug info\n3. Observe if only Silver hands are highlighted",

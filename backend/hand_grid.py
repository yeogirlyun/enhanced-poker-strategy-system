...
class HandGridWidget:
    ...
    def __init__(self, parent_frame, strategy_data: StrategyData, on_hand_click: Callable = None):
        ...
        self.grid_size_index = 6  # Default to "Giant" (fills 75%+ of grid window)
    ...
    def _setup_ui(self):
        ...
        self.grid_canvas = tk.Canvas(self.main_frame, bg=THEME["bg"], highlightthickness=0)
        self.grid_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        # Create scrollbar
        self.scrollbar = ttk.Scrollbar(self.main_frame, orient=tk.VERTICAL, command=self.grid_canvas.yview)
        ...
        # Bind canvas resize
        self.hand_frame.bind('<Configure>', lambda e: self.grid_canvas.configure(scrollregion=self.grid_canvas.bbox('all')))
    ...
    def _render_grid(self):
        ...
        print(f"DEBUG: Current state: tiers={len(self._grid_state['selected_tiers'])}, hands={len(self._grid_state['selected_hands'])}")
        ...
        for i, rank in enumerate(ranks):
            label = tk.Label(self.hand_frame, text=rank, 
                           font=self.base_font, 
                           bg='#006400', fg='#ffffff',  # Dark green with white text
                           width=3, height=1)
        ...
        for i, rank1 in enumerate(ranks):
            for j, rank2 in enumerate(ranks):
                ...
                bg_color, fg_color, border_color, border_width = self._get_button_appearance(hand)
                
                # Create button with HS score
                button_frame = self._create_button_with_hs(hand, size_config, bg_color, fg_color, border_color, border_width)
                
                # Add click command to the frame
                button_frame.bind('<Button-1>', lambda e, h=hand: self.toggle_hand_selection(h))
                main_label = button_frame.winfo_children()[0]  # Get the main label
                main_label.bind('<Button-1>', lambda e, h=hand: self.toggle_hand_selection(h))
            ...
            for tier in self._grid_state['selected_tiers']:
                ...
                for tier in self.strategy_data.tiers:
                    if hand in tier.hands:
                        ...
                        bg_color = self._dim_color(tier.color, 0.3)  # Dim the color by 70%
    ...
    def toggle_hand_selection(self, hand: str):
        ...
        print(f"DEBUG: Hand '{hand}' {'selected' if hand in self._grid_state['selected_hands'] else 'deselected'}")
        
        # Update state and trigger redraw
        self._update_grid_state(selected_hands=self._grid_state['selected_hands'].copy())
    ...
    def _create_button_with_hs(self, hand: str, size_config: Dict, bg_color: str, fg_color: str, border_color: str, border_width: int):
        ...
        hand_label = tk.Label(button_frame, text=hand, font=size_config['font'], 
                            bg=bg_color, fg=fg_color, padx=2, pady=1)
        hand_label.pack(expand=True, fill=tk.BOTH)
        
        # HS score label (only for larger fonts)
        ...
        if hs_score is not None and size_config['font'][1] > 10:
            hs_label = tk.Label(button_frame, text=str(hs_score), 
                              font=(size_config['font'][0], size_config['font'][1] - 2),

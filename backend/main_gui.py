...
class PokerStrategyGUI:
    ...
    def _setup_styles(self, button_font=("Arial", 12, "bold")):
        ...
        current_font_size = GridSettings.get_size_config(
            self.font_sizes[self.current_font_size_index]
        )["font"][1]

        # Light sky blue colors for tabs and text fields
        light_sky_blue = "#87CEEB"  # Light sky blue
        lighter_sky_blue = "#B0E0E6"  # Even lighter sky blue for text fields
        ...
        # Notebook (tab) styles with light sky blue
        style.configure("TNotebook", background=light_sky_blue)
        style.configure(
            "TNotebook.Tab",
        ...
        style.map(
            "TNotebook.Tab",
            background=[("selected", lighter_sky_blue), ("active", lighter_sky_blue)],
            foreground=[("selected", "#000"), ("active", "#000")],
        )

        # Text field styles with light sky blue
        style.configure(
            "SkyBlue.TEntry",
            fieldbackground=lighter_sky_blue,
            background=lighter_sky_blue,
            foreground="#333",
            borderwidth=2,
            relief="solid",
            font=(THEME["font_family"], current_font_size),
        ...
        # Combobox styles with light sky blue
        style.configure(
            "SkyBlue.TCombobox",
            fieldbackground=lighter_sky_blue,
            background=lighter_sky_blue,
        ...
        style.map(
            "SkyBlue.TCombobox",
            fieldbackground=[
                ("readonly", lighter_sky_blue),
                ("active", lighter_sky_blue),
            ],
            background=[("readonly", lighter_sky_blue), ("active", lighter_sky_blue)],
        )
        ...
        style.configure(
            "TopArea.TLabel",
            background=top_area_bg,
            foreground=top_area_fg,
            font=(THEME["font_family"], current_font_size),
        )
        style.configure(
            "TopArea.TLabelframe", background=top_area_bg, bordercolor=top_area_border
        )
        style.configure(
            "TopArea.TLabelframe.Label",
            background=top_area_bg,
            foreground=top_area_fg,
            font=(THEME["font_family"], current_font_size),
        )
    ...
    def _setup_menu(self):
        ...
        tools_menu.add_command(
            label="Clear All Highlights", command=self._clear_all_highlights
        )
    ...
    def _setup_ui(self):
        ...
        top_frame = ttk.LabelFrame(
            self.root, text="Application Controls", style="TopArea.TLabelframe"
        )
        ...
        # Strategy file info
        ttk.Label(controls_container, text="Strategy:", style="TopArea.TLabel").grid(
            row=0, column=2, padx=1, pady=0, sticky="nsew"
        )
        self.strategy_label = ttk.Label(
            controls_container, text="Default Strategy", style="TopArea.TLabel"
        )
        ...
        ttk.Label(controls_container, text="App Font:", style="TopArea.TLabel").grid(
            row=0, column=8, padx=5, pady=0, sticky="nsew"
        )
        ...
        self.clear_highlights_btn = ttk.Button(
            controls_container,
            text="Clear Highlights",
            command=self._clear_all_highlights,
            style="TopMenu.TButton",
        )
        ...
        self.hand_grid = HandGridWidget(
            grid_frame, self.strategy_data, self._on_hand_click
        )
    ...
    def _show_statistics(self):
        ...
        stats_text += (
            f"Current Strategy: {self.strategy_data.get_strategy_file_display_name()}\n"
        )
        if current_file:
        ...
        stats_text += (
            f"Current Font Size: {self.font_sizes[self.current_font_size_index]}"
        )
    ...
    def _new_strategy(self):
        ...
        if messagebox.askyesno(
            "New Strategy",
            "Are you sure you want to create a new strategy? This will clear all current data.",
        ):
            self.strategy_data = StrategyData()
            self.strategy_data.load_default_tiers()
            self.hand_grid._render_grid()
    ...
    def _open_strategy(self):
        ...
        if not available_files:
            messagebox.showinfo(
                "No Strategy Files", "No strategy files found in the current directory."
            )
            return
        ...
        # File list
        listbox = tk.Listbox(
            dialog, bg=THEME["bg_dark"], fg=THEME["fg"], font=(THEME["font_family"], 12)
        )
        ...
        ttk.Button(
            button_frame, text="Load", command=load_selected, style="Dark.TButton"

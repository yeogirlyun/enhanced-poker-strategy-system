...
class PurePokerStateMachine:
                ...
                if self.decision_engine and self.decision_engine.has_decision_for_player(current_player.name):
                    try:
                        ...
                        if decision:
                            ...
                            # No decision available for current player, try advancing
                            # This handles cases where hand model action order differs from PPSM
                    ...
                    # No decision engine or no decision available, try advancing
                    # This handles cases where action order needs adjustment
                    self._advance_to_next_player()
                    continue
            else:
    ...
    def _resolve_showdown(self):
        """Resolve the showdown and determine winners."""
        ...
        # For hands validation we don't rank; we just finalize
        print(f"🃏 PPSM: Resolving showdown, final pot: ${self.game_state.displayed_pot()}")
        self.current_state = PokerState.SHOWDOWN
        # (emit showdown events / hand ranking here if/when implemented)
...
class HandModelDecisionEngineAdapter:
    ...
    def get_decision(self, player_name: str, game_state) -> tuple:
        ...
        # Find the next action for this player (skip actions for other players)
        while self.current_action_index < len(self.actions_for_replay):
            ...
            if current_action.actor_uid == player_name:
                # We will CONSUME this action, so advance the pointer now.
                self.current_action_index += 1
                from core.hand_model import ActionType as HM
                # Fast access to PPSM state
                curr = game_state.current_bet
                bb   = 10.0  # Default big blind for calculations
                # Find this player to compute stack headroom
                p = next((pl for pl in game_state.players if pl.name == player_name), None)
                stack_room = (p.stack + p.current_bet) if p else float("inf")

                if current_action.action == HM.CHECK:
                    return ActionType.CHECK, None
                if current_action.action == HM.FOLD:
                    return ActionType.FOLD, None
                if current_action.action == HM.CALL:
                    # ignore any numeric value; PPSM will pay to current_bet
                    return ActionType.CALL, None

                if current_action.action == HM.BET:
                    amt = float(current_action.amount or 0.0)
                    if curr == 0:
                        # First wager of street → BET(to = amt)
                        return ActionType.BET, amt
                    else:
                        # Hand model mislabeled a raise as a bet; convert to RAISE(to)
                        # Prefer full-raise interpretation when possible
                        to_total = curr + amt
                        if to_total > curr and to_total <= stack_room:
                            return ActionType.RAISE, to_total
                        # Fallback: treat as total if it already looks like one
                        if amt > curr and amt <= stack_room:
                            return ActionType.RAISE, amt
                        # Last resort: return None (validator will surface a clear error)
                        return None

                if current_action.action == HM.RAISE:
                    r = float(current_action.amount or 0.0)
                    # Try both interpretations: delta and total. Prefer a "full-raise-ish" total.
                    candidates = []
                    # As delta:
                    total1 = curr + r
                    ...
                    # Prefer a candidate with at least bb over-current (full-ish), else any valid
                    chosen = None
                    for t in candidates:
                        if (t - curr) + 1e-9 >= bb:
                            chosen = t
                ...
                # Not this player's turn in the hand model; DO NOT consume it.
                return None
    ...
    def has_decision_for_player(self, player_name: str) -> bool:
        ...
        # Look ahead to find the next action for this player
        for i in range(self.current_action_index, len(self.actions_for_replay)):
            action = self.actions_for_replay[i]
            if action.actor_uid == player_name:
                return True

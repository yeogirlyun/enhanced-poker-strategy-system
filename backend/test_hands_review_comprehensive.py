...
try:
    from core.hands_review_poker_state_machine_new import (
...
class HandsReviewTester:
    ...
    def __init__(self):
        self.session_manager = SessionManager(
            num_players=9, big_blind=100.0
        )  # Default poker table setup
    ...
    def setup_hands_review(self) -> bool:
        ...
        try:
            ...
            self.state_machine = HandsReviewPokerStateMachine(
                config, session_logger=self.logger
            )
            self.state_machine.set_game_director(self.game_director)

            # Register with game director (use None for UI components in
            # headless test)
    ...
    def load_legendary_hands(self) -> List[Dict[str, Any]]:
        ...
        try:
            hands_file = os.path.join(
                os.path.dirname(__file__),
                "data",
                "legendary_hands_complete_130_fixed.json",
            )
            ...
            if isinstance(hands_data, dict) and "hands" in hands_data:
                ...
                print("❌ Unexpected hands data format")
                return []
    ...
    def mock_step_forward_clicks(
        self, hand_data: Dict[str, Any], max_steps: int = 100
    ) -> HandTestResult:
        ...
        try:
            ...
            for step in range(max_steps):
                current_action_index = getattr(
                    self.state_machine, "action_index", 0
                )

                # Check if we've completed all actions
                if current_action_index >= total_actions:
                    print(
                        f"✅ Hand completed - all {total_actions} actions executed"
                    )
                ...
                if current_action_index == last_action_index:
                    ...
                    if consecutive_failures >= 3:
                        stalling_pattern = self._identify_stalling_pattern(
                ...
                if step % 5 == 0:  # Progress update every 5 steps
                    print(
                        f"   Step {step + 1}: Action {current_action_index + 1}/{total_actions} - {'✅' if success else '❌'}"
                    )
    ...
    def _identify_stalling_pattern(self, action_index: int) -> str:
        ...
        try:
            ...
            if (
                hasattr(self.state_machine, "historical_actions")
                and self.state_machine.historical_actions
            ):
            ...
            if current_action:
                ...
                elif "Player" in str(current_state) and "DEAL" in str(
                    current_state
                ):
    ...
    def test_all_hands(self) -> None:
        ...
        for i, hand_data in enumerate(legendary_hands[:2]):
            ...
            print(
                f"{status_emoji[result.result]} {result.result.value.upper()}: "
    ...
    def _generate_report(self) -> None:
        ...
        successful = len(
            [r for r in self.test_results if r.result == TestResult.SUCCESS]
        )
        stalled = len(
            [r for r in self.test_results if r.result == TestResult.STALLED]
        )
        errors = len(
            [r for r in self.test_results if r.result == TestResult.ERROR]
        )
        incomplete = len(
            [r for r in self.test_results if r.result == TestResult.INCOMPLETE]
        )
        ...
        if stalled > 0:
            ...
            for pattern, hand_ids in stalling_patterns.items():
                print(
                    f"   {pattern}: {len(hand_ids)} hands ({', '.join(hand_ids[:5])}{'...' if len(hand_ids) > 5 else ''})"
                )
        ...
        if incomplete > 0:
            print(
                f"   3. Investigate incomplete executions - {incomplete} hands affected"
            )

        if successful == total_hands:
            print(
                "   🎉 All hands completed successfully! Ready for production."
            )
                ...
                "summary": {
                    ...
                    "successful": len(
                        [
                            ...
                            if r.result == TestResult.SUCCESS
                        ]
                    ),
                    "stalled": len(
                        [
                            r
                            for r in self.test_results
                            if r.result == TestResult.STALLED
                        ]
                    ),
                    "errors": len(
                        [
                    ...
                    "incomplete": len(
                        [
                            r
                            for r in self.test_results
                            if r.result == TestResult.INCOMPLETE
                        ]
                    ),
                },
                "detailed_results": [
                    {
                        ...
                        "final_state": (
                            str(r.final_state) if r.final_state else None
                        ),

...
class ConsolidatedFlexiblePokerStateMachineTest(unittest.TestCase):
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(ValueError):
            self.state_machine.transition_to(PokerState.SHOWDOWN)  # Invalid from START_HAND
    ...
    def test_all_players_all_in_preflop(self):
        ...
        winners = self.state_machine.determine_winners(self.state_machine.game_state.players)
        self.assertIsInstance(winners, list)
    ...
    def test_single_player_remaining(self):
        ...
        for i, player in enumerate(self.state_machine.game_state.players[1:], 1):
            player.has_folded = True
        
        # Test that remaining player wins
        active_players = [p for p in self.state_machine.game_state.players if not p.has_folded]
    ...
    def test_pot_equals_investments(self):
        ...
        total_investments = sum(p.current_bet for p in self.state_machine.game_state.players)
    ...
    def test_no_negative_stacks(self):
        ...
        if action_player:
            ...
            self.state_machine.execute_action(action_player, ActionType.CALL, action_player.stack)
    ...
    def test_multiple_all_ins_different_stacks(self):
        ...
        for player in self.state_machine.game_state.players:
            if player.stack > 0:
                self.state_machine.execute_action(player, ActionType.RAISE, player.stack)
                break
        
        # Check that all-ins are handled correctly
        all_in_players = [p for p in self.state_machine.game_state.players if p.is_all_in]
...
def run_consolidated_tests():
    ...
    suite = loader.loadTestsFromTestCase(ConsolidatedFlexiblePokerStateMachineTest)

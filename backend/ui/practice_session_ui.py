...
from core.practice_session_poker_state_machine import (
    PracticeSessionPokerStateMachine,
)
from core.practice_session_action_interface import (
    PracticeSessionActionInterface,
)
...
from ui.components.practice_session_poker_widget import (
...
class PracticeSessionUI(ttk.Frame, EventListener):
    ...
    def __init__(
        self, parent, strategy_data: StrategyData, poker_config=None, **kwargs
    ):
        ...
        try:
            ...
            self.logger.log_system(
                "INFO",
                "PRACTICE_UI_INIT",
                "PracticeSessionUI initialization started",
        ...
        if poker_config:
            ...
            if self.logger:
                self.logger.log_system(
                    "INFO",
                    "PRACTICE_UI_INIT",
                    "Using provided poker configuration",
                    {
            ...
            # Fallback to defaults if no config provided (use standard
            # GameConfig defaults)
            config = GameConfig(
                num_players=6,
            ...
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "PRACTICE_UI_CONFIG",
                    f"Using default config: starting_stack={
            ...
            if self.logger:
                self.logger.log_system(
                    "INFO",
                    "PRACTICE_UI_INIT",
                    "Using default poker configuration",
                    {
                        "num_players": config.num_players,
                        "big_blind": config.big_blind,
                        "small_blind": config.small_blind,
        ...
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_UI_INIT",
                "Creating PracticeSessionPokerStateMachine",
                {
                    "num_players": config.num_players,
                    "big_blind": config.big_blind,
                    "small_blind": config.small_blind,
        ...
        try:
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "PRACTICE_UI_CONFIG",
                    f"Creating state machine with config starting_stack={
                        config.starting_stack}",
                    {},
                )
            self.state_machine = PracticeSessionPokerStateMachine(
            ...
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "PRACTICE_UI_CONFIG",
                    "State machine created, checking player stacks",
                    {},
                )
            ...
            if self.logger:
                self.logger.log_system(
                    "INFO",
                    "PRACTICE_UI_INIT",
                    "PracticeSessionPokerStateMachine created successfully",
                    {},
                )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_UI_INIT",
                    f"Failed to create PracticeSessionPokerStateMachine: {e}",
        ...
        if self.logger:
            self.logger.log_system(
                "INFO", "PRACTICE_UI_INIT", "Adding UI as event listener", {}
            )
        self.state_machine.add_event_listener(self)

        # Initialize clean action interface
        self.action_interface = PracticeSessionActionInterface(
            self.state_machine
        )
        ...
        if self.logger:
            self.logger.log_system(
                "INFO", "PRACTICE_UI_INIT", "Setting up UI components", {}
            )
        self._setup_ui()
        ...
        if self.logger:
            ...
            self.logger.log_system(
                "INFO", "PRACTICE_UI_INIT", "Practice Session UI ready", {}
            )
    ...
    def on_event(self, event: GameEvent):
        ...
        try:
            ...
            if event.event_type == "practice_hand_started":
                ...
                if hasattr(self, "poker_widget") and hasattr(
                    self.poker_widget, "on_event"
                ):
                ...
                self._add_action_message(
                    "✅ Hand complete. Click START NEW HAND to continue."
                )
    ...
    def _setup_ui(self):
        ...
        # Configure main layout - poker table takes full width, bottom strip
        # for controls
        self.grid_rowconfigure(0, weight=1)  # Poker table area (expandable)
        # Bottom control strip (fixed height)
        ...
        # CRITICAL: Register poker widget as event listener to receive
        # hand_complete events
        self.state_machine.add_event_listener(self.poker_widget)
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_UI_INIT",
                "Registered poker widget as event listener for hand_complete events",
                {
                    "widget_type": type(self.poker_widget).__name__,
                    "widget_has_on_event": hasattr(
                        self.poker_widget, "on_event"
                    ),
                    "total_listeners": len(
                        getattr(self.state_machine, "_event_listeners", [])
                    ),
                },
            )
    ...
    def _setup_bottom_control_strip(self):
        ...
        separator = tk.Frame(
            bottom_frame, bg="#2E4F76", height=2  # Dark Steel Blue border
        )
        separator.pack(fill="x", side="top")

        # LV-style gold hairline (subtle)
        gold_hairline = tk.Frame(bottom_frame, bg="#FFD700", height=1)
        ...
        if os.path.exists(lv_path):
            ...
            if self._lv_bg_image is not None:
                ...
                def _tile_bg(event=None):
                    ...
                    iw, ih = (
                        self._lv_bg_image.width(),
                        self._lv_bg_image.height(),
                    )
        ...
        content_frame.grid_columnconfigure(
            0, weight=1, minsize=320
        )  # Message area (expandable)
        content_frame.grid_columnconfigure(
            1, weight=0, minsize=360
        )  # Action buttons cluster
        content_frame.grid_columnconfigure(
            2, weight=0, minsize=320
        )  # Bet buttons cluster
        content_frame.grid_rowconfigure(
            0, weight=1, minsize=80
        )  # Consistent panel height

        # === LEFT: Game Message Area (now column 0) ===
        message_frame = tk.Frame(
            content_frame, bg="#1E232A"  # Match panel background
        )
    ...
    def _setup_session_controls_bottom(self, parent):
        ...
        self._cluster_check_frame, self._cluster_check_label = create_button(
            ...
            "CHECK",
            "#3980A6",
            lambda: self._handle_action_click("check_call"),
        )
        self._cluster_check_frame.grid(
        ...
        self._cluster_fold_frame, self._cluster_fold_label = create_button(
            ...
            "FOLD",
            "#757575",
            lambda: self._handle_action_click("fold"),
        )
        self._cluster_fold_frame.grid(
        ...
        start_container = tk.Frame(
            ...
            highlightbackground="#2E4F76",
            highlightcolor="#2E4F76",
        )
        ...
        self.button_configs = {
            "check": {
                "text": "CHECK",
                "color": self.original_button_colors["check_call"],
            },
            "call": {
                "text": "CALL",
                "color": self.original_button_colors["check_call"],
        ...
        try:
            ...
            if hasattr(self, "poker_widget") and self.poker_widget:
                if hasattr(self.poker_widget, "change_table_scheme"):
                    ...
                    self.poker_widget.change_table_scheme(
            ...
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "PRACTICE_UI_TABLE",
                    f"Could not initialize table felt: {e}",
                    {},
                )
    ...
    def _add_action_message(self, message: str):
        ...
        if hasattr(self, "action_message_text"):
            ...
            current_content = self.action_message_text.get(
                "1.0", tk.END
            ).strip()
            ...
            # Ensure we always have exactly 2 lines (pad with empty line if
            # needed)
            while len(lines) < 2:
                # Pad at the beginning for better centering
                lines.insert(0, "")
    ...
    def _handle_action_message(self, event):
        ...
        if hasattr(event, "data") and event.data:
            ...
            if action == "fold":
                ...
                if amount > 0:
                    self._add_action_message(
                        f"📞 {player_name} calls ${amount:.0f}"
                    )
                else:
                    self._add_action_message(f"📞 {player_name} calls")
            elif action == "bet":
                self._add_action_message(
                    f"💰 {player_name} bets ${amount:.0f}"
                )
            elif action == "raise":
                self._add_action_message(
                    f"🚀 {player_name} raises to ${
                        amount:.0f}"
                )
            elif action == "all_in":
                self._add_action_message(
                    f"🔥 {player_name} goes ALL IN (${amount:.0f})"
                )
            else:
    ...
    def _handle_street_change_message(self, event):
        ...
        if hasattr(event, "data") and event.data:
            ...
            if "DEAL_FLOP" in new_state:
                # Get board info using action interface (no direct state
                # access)
    ...
    def _handle_hand_complete_message(self, event):
        ...
        if hasattr(event, "data") and event.data:
            # FIXED: Use the winners list from event data instead of parsing
            # winner_info['name']
            winners_list = event.data.get("winners", [])
            winner_info = event.data.get("winner_info", {})
            pot_amount = event.data.get("pot_amount", 0)
            hand_desc = winner_info.get("hand_description") or event.data.get(
                "winning_hand", ""
            )
            ...
            if isinstance(winners_list, list) and len(winners_list) > 1:
                ...
                if best_five:
                    ...
                    self._add_action_message(
                        f"🏆 Split pot: {winners_str} each win ${
                            share:.2f} with {hand_desc} ({best_five_str})"
                    )
                else:
                    self._add_action_message(
                        f"🏆 Split pot: {winners_str} each win ${
                            share:.2f} with {hand_desc}"
                    )
            else:
                # Single winner
                winner = (
                    winners_list[0]
                    if winners_list
                    else (winner_info.get("name") or "Unknown")
                )
                if best_five:
                    best_five_str = " ".join(best_five)
                    self._add_action_message(
                        f"🏆 {winner} wins ${
                            pot_amount:.2f} with {hand_desc} ({best_five_str})"
                    )
                else:
                    self._add_action_message(
                        f"🏆 {winner} wins ${
                            pot_amount:.2f} with {hand_desc}"
                    )
    ...
    def _setup_quick_bet_buttons(self, parent):
        ...
        for text, key, row, col in bet_buttons_data:
            ...
            # Create frame
            bet_frame = tk.Frame(
                bet_panel, bg=base_color, relief="flat", bd=0, cursor="hand2"
            )
    ...
    def _setup_action_buttons(self, parent):
        ...
        # Professional button container with subtle LV background pattern (2%
        # opacity simulated)
        ...
        check_frame.bind(
            "<Button-1>", lambda e: self._handle_action_click("check_call")
        )
        check_label.bind(
            "<Button-1>", lambda e: self._handle_action_click("check_call")
        )
        ...
        check_frame.bind("<Key>", lambda e: "break")
        # Block all keyboard events
        check_label.bind("<Key>", lambda e: "break")
        ...
        # Bind click events to both frame and label with focus protection
        fold_frame.bind(
            "<Button-1>", lambda e: self._handle_action_click("fold")
        )
        fold_label.bind(
            "<Button-1>", lambda e: self._handle_action_click("fold")
        )
        ...
        # DISABLE KEYBOARD TRIGGERS to prevent accidental activation
        # Block all keyboard events
        fold_frame.bind("<Key>", lambda e: "break")
        # Block all keyboard events
        fold_label.bind("<Key>", lambda e: "break")
        ...
        self.action_buttons = {"check_call": check_button, "fold": fold_button}
        ...
        self.action_labels = {"check_call": check_label, "fold": fold_label}
        ...
        self.button_configs = {
            ...
            "call": {
                "text": "CALL",
                "color": THEME.get("button_call", THEME["button_check"]),
            },
        }
    ...
    def _disable_action_buttons(self):
        ...
        for button_key, button_frame in self.action_buttons.items():
            if hasattr(button_frame, "config"):
                button_frame.config(
                    bg=disabled_color, cursor="", relief="flat"
                )
            ...
            if button_key in self.action_button_labels:
                ...
                if hasattr(label, "config"):
                    label.config(
                        bg=disabled_color, fg=disabled_text_color, cursor=""
                    )
    ...
    def _enable_action_buttons(self):
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "UI_STATE",
                "Enabling action buttons for human turn",
                {},
            )

        for button_key, button_frame in self.action_buttons.items():
            # Restore original colors
            original_color = self.original_button_colors.get(
                button_key, THEME["button_check"]
            )

            if hasattr(button_frame, "config"):
                button_frame.config(
                    bg=original_color, cursor="hand2", relief="raised"
                )
            ...
            if button_key in self.action_button_labels:
                self.action_button_labels[button_key].bind(
                    "<Button-1>", handler
                )
    ...
    def _check_and_enable_human_turn(self):
        ...
        try:
            ...
            if self.action_interface.can_human_act():
                ...
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "UI_STATE",
                        "Human turn - enabling action buttons",
                        {"component": "PracticeSessionUI"},
                    )
            else:
                self._disable_action_buttons()
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "UI_STATE",
                        "Not human turn - disabling action buttons",
                        {"component": "PracticeSessionUI"},
                    )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR", "UI_STATE", f"Error checking human turn: {e}", {}
                )
    ...
    def _handle_action_click(self, action_key: str):
        ...
        if hasattr(self, "_last_action_time") and hasattr(
            ...
            if time_since_last < 0.5:
                print(
                    f"🛡️ AUTO-CLICK PROTECTION: Action blocked (too fast: {time_since_last:.3f}s)"
                )
                if self.logger:
                    self.logger.log_system(
                        "WARNING",
                        "PRACTICE_UI_ACTION",
                        f"Action blocked - too fast: {
                            time_since_last:.3f}s",
                        {"action": action_key},
                    )
                return

            # Block repeated identical actions within 2 seconds (likely
            # accidental/auto-clicks)
            if self._last_action_key == action_key and time_since_last < 2.0:
                print(
                    f"🛡️ REPEAT-CLICK PROTECTION: Identical action blocked (repeated {action_key} within {
                        time_since_last:.3f}s)"
                )
                if self.logger:
                    self.logger.log_system(
                        "WARNING",
                        "PRACTICE_UI_ACTION",
                        f"Action blocked - repeated {action_key} within {
                            time_since_last:.3f}s",
                        {"action": action_key},
                    )
                return
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "PRACTICE_UI_ACTION",
                f"Action button clicked: {action_key}",
        ...
        if not self.action_interface.can_execute_action(action_key):
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "PRACTICE_UI_ACTION",
                    f"Action blocked by interface: {action_key}",
                    {},
                )
            return
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "PRACTICE_UI_ACTION",
                f"Human player's turn, executing action: {action_key}",
                {},
            )
        ...
        if action_key == "check_call":
            ...
            # Use current bet amount if set by quick-bet, otherwise interface
            # calculates default
        ...
        # Disable buttons after action (they'll be re-enabled when it's human's
        # turn again)
        self._disable_action_buttons()
    ...
    def _setup_educational_panel(self, parent):
        ...
        edu_frame = ttk.LabelFrame(
            parent, text="Educational Features", padding=10
        )
        ...
        edu_scrollbar = ttk.Scrollbar(
            edu_frame, orient=tk.VERTICAL, command=self.edu_text.yview
        )
    ...
    def _setup_statistics_panel(self, parent):
        ...
        stats_frame = ttk.LabelFrame(
            parent, text="Session Statistics", padding=10
        )
    ...
    def _start_new_hand(self):
        ...
        try:
            if self.logger:
                self.logger.log_system(
                    "INFO", "PRACTICE_HAND", "Starting new practice hand", {}
                )
            self.state_machine.start_hand()

            # Update UI state
            self._disable_start_button()
            self._add_educational_message(
                "🎯 New hand started! Observe the cards and consider your position."
            )
    ...
    def _reset_session(self):
        ...
        if messagebox.askyesno(
            "Reset Session",
            "Are you sure you want to reset the practice session?",
        ):
            try:
                # Reset state machine
                ...
                if self.logger:
                    self.logger.log_system(
                        "ERROR", "SESSION", f"Error resetting session: {e}", {}
                    )
    ...
    def _get_strategy_hint(self):
        ...
        try:
            ...
            if human_player:
                ...
                if suggestion:
                    self._add_educational_message(
                        f"💡 Strategy Hint: {suggestion}"
                    )
                else:
                    self._add_educational_message(
                        "💡 No specific hint available for current situation."
                    )
            else:
                self._add_educational_message(
                    "💡 Start a hand first to get strategy hints."
                )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_EDU",
                    f"Error getting strategy hint: {e}",
                    {},
                )
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_EDU",
                    f"Error analyzing current hand: {e}",
                    {},
                )
    ...
    def _handle_hand_started(self, event: GameEvent):
        ...
        if event.data:
            ...
            message = (
                f"🎯 Hand #{hand_num} started! You are in {position} position."
            )
...
Decision Accuracy: {
                                stats['decision_accuracy']:.1f}%

Total Winnings: ${
                                    stats['total_winnings']:.2f} """
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "SESSION",
                    f"Error updating session stats: {e}",
                    {},
                )
    ...
    def update_poker_config(self, new_config):
        ...
        try:
            ...
            self._add_educational_message(
                f"🔧 Updated game settings: ${
                    new_config.small_blind}/${
                    new_config.big_blind} blinds, ${
                    new_config.starting_stack} starting stack"
            )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_UI_CONFIG",
                    f"Error updating poker configuration: {e}",
                    {},
                )
    ...
    def update_font_size(self, font_size: int):
        ...
        try:
            ...
            if (
                hasattr(self, "action_message_text")
                and self.action_message_text
            ):
            ...
            if hasattr(self, "poker_widget") and hasattr(
                self.poker_widget, "update_font_size"
            ):
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_UI",
                    f"Error updating font size: {e}",
                    {},
                )

    def _analyze_deviation_type(
        self, user_action: str, gto_action: str
    ) -> str:
        ...
        if user_action in aggressive_actions and gto_action in passive_actions:
            ...
            user_action in passive_actions and gto_action in aggressive_actions
        ):
            return "PASSIVE"
    ...
    def increase_table_size(self):
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "PRACTICE_UI_TABLE_SIZE",
                "increase_table_size() called",
                {},
            )
        if hasattr(self, "poker_widget") and self.poker_widget:
            if hasattr(self.poker_widget, "increase_table_size"):
                ...
                if self.logger:
                    self.logger.log_system(
                        "WARNING",
                        "PRACTICE_UI_TABLE_SIZE",
                        "poker_widget missing increase_table_size method",
                        {},
                    )
        else:
            if self.logger:
            ...
            if hasattr(self.poker_widget, "decrease_table_size"):
                ...
                if self.logger:
                    self.logger.log_system(
                        "WARNING",
                        "PRACTICE_UI_TABLE_SIZE",
                        "poker_widget missing decrease_table_size method",
                        {},
                    )
        else:
            if self.logger:
    ...
    def _update_action_buttons_for_game_state(self):
        ...
        try:
            ...
            current_bet = float(
                getattr(self.state_machine.game_state, "current_bet", 0) or 0
            )
            ...
            if call_amount > 0:
                ...
                # Bet/Raise handled via preset grid; no primary action button
                # needed
            else:
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_UI",
                    f"Error updating action buttons: {e}",
                    {},
                )
    ...
    def _update_check_call_button(self, action_type: str, amount: float = 0):
        ...
        label = self.action_button_labels.get(
            "check_call"
        ) or self.action_labels.get("check_call")

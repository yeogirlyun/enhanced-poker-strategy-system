...
class TierPanel:
    ...
    def __init__(
        ...
        on_tier_change: Callable = None,
        on_tier_select: Callable = None,
    ):
    ...
    def _setup_ui(self):
        ...
        self.main_frame = ttk.LabelFrame(
            self.parent,
            text="Tiers & Strategy Management",
        ...
        tier_scrollbar = ttk.Scrollbar(
            list_frame, orient=tk.VERTICAL, command=self.tier_listbox.yview
        )
        ...
        # Use TopMenu.TButton style for consistency with top menu, but maintain
        # current size
        self.add_tier_btn = ttk.Button(
            button_frame,
            text="Add Tier",
            command=self._add_tier,
            style="TopMenu.TButton",
            width=10,
        )
        self.add_tier_btn.pack(side=tk.LEFT, padx=2)

        self.remove_tier_btn = ttk.Button(
            button_frame,
            text="Remove",
            command=self._remove_tier,
            style="TopMenu.TButton",
            width=10,
        )
        self.remove_tier_btn.pack(side=tk.LEFT, padx=2)

        self.edit_tier_btn = ttk.Button(
            button_frame,
            text="Edit",
            command=self._edit_tier,
            style="TopMenu.TButton",
            width=10,
        )
        ...
        count_frame = ttk.LabelFrame(
            self.main_frame, text="Statistics", style="Dark.TLabelframe"
        )
        count_frame.pack(fill=tk.X, padx=5, pady=3)

        # Total playable hands count (compact)
        self.total_hands_label = ttk.Label(
            count_frame, text="Total Playable Hands: 0", style="Dark.TLabel"
        )
        self.total_hands_label.pack(anchor=tk.W, padx=5, pady=2)

        # Selected hand frame
        self.selected_hand_frame = ttk.LabelFrame(
            self.main_frame, text="Selected Hand", style="Dark.TLabelframe"
        )
        self.selected_hand_frame.pack(fill=tk.X, padx=5, pady=3)

        # Selected hand info
        self.selected_hand_label = ttk.Label(
            self.selected_hand_frame,
            text="Click a hand in the grid to edit its HS score",
            style="Dark.TLabel",
        )
        self.selected_hand_label.pack(anchor=tk.W, padx=5, pady=2)

        # HS score editing frame
        self.hs_edit_frame = ttk.Frame(
            self.selected_hand_frame, style="Dark.TFrame"
        )
        self.hs_edit_frame.pack(fill=tk.X, padx=5, pady=2)

        ttk.Label(
            self.hs_edit_frame, text="HS Score:", style="Dark.TLabel"
        ).pack(side=tk.LEFT, padx=(0, 5))
        self.hs_score_var = tk.StringVar()
        self.hs_score_entry = ttk.Entry(
            self.hs_edit_frame,
            textvariable=self.hs_score_var,
            width=8,
            style="SkyBlue.TEntry",
        )
        self.hs_score_entry.pack(side=tk.LEFT, padx=(0, 5))

        self.update_hs_btn = ttk.Button(
            self.hs_edit_frame,
            text="Update HS",
            command=self._update_hs_score,
            style="TopMenu.TButton",
        )
        ...
        self.selected_count_label = ttk.Label(
            count_frame, text="Selected Cards: 0", style="Dark.TLabel"
        )
        self.selected_count_label.pack(anchor=tk.W, padx=5, pady=2)

        # Tier details frame (more space for details)
        details_frame = ttk.LabelFrame(
            self.main_frame, text="Tier Details", style="Dark.TLabelframe"
        )
        details_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=3)

        self.tier_details_label = ttk.Label(
            details_frame,
            text="Select a tier to view details",
            style="Dark.TLabel",
            wraplength=250,
        )
        self.tier_details_label.pack(
            anchor=tk.NW, padx=5, pady=5, fill=tk.BOTH, expand=True
        )
    ...
    def _update_tier_list(self):
        ...
        for tier in self.strategy_data.tiers:
            self.tier_listbox.insert(
                tk.END,
                f"{tier.name} (HS {tier.min_hs}-{tier.max_hs}) - {len(tier.hands)} hands",
            )
    ...
    def _update_counts(self):
        ...
        self.total_hands_label.configure(
            text=f"Total Playable Hands: {total_hands}"
        )
        ...
        self.selected_count_label.configure(
            text=f"Selected Cards: {len(selected_hands)}"
        )
    ...
    def _update_tier_details(self):
        ...
        if len(self.selected_tiers) == 1:
            ...
            details_text = "Multiple Tiers Selected:\n"
    ...
    def _remove_tier(self):
        ...
        selected_indices = sorted(
            list(self.tier_listbox.curselection()), reverse=True
        )
        if not selected_indices:
            messagebox.showwarning(
                "No Selection", "Please select one or more tiers to remove."
            )
            return

        # Confirm deletion
        tier_names = [
            self.strategy_data.tiers[i].name for i in selected_indices
        ]
    ...
    def _edit_tier(self):
        ...
        if len(selected_indices) != 1:
            messagebox.showwarning(
                "Selection Error", "Please select exactly one tier to edit."
            )
            return
        ...
        if dialog.result:
            ...
            # Check for overlapping HS ranges (excluding the current tier being
            # edited)
            has_overlap = False
            overlapping_tiers = []
            for i, existing_tier in enumerate(self.strategy_data.tiers):
                if i != index:  # Skip the tier being edited
                    if (
                        temp_tier.min_hs <= existing_tier.max_hs
                        and temp_tier.max_hs >= existing_tier.min_hs
                    ):
                        has_overlap = True
            ...
            if has_overlap:
                ...
                conflict_msg = f"HS range {
                    temp_tier.min_hs}-{
                    temp_tier.max_hs} overlaps with existing tiers:\n\n"
                for tier in overlapping_tiers:
                    conflict_msg += (
                        f"• {tier.name} (HS {tier.min_hs}-{tier.max_hs})\n"
                    )

                conflict_msg += "\nChoose an option:\n"
                conflict_msg += (
                    "• 'Auto-adjust': Automatically adjust the tier's range\n"
                )
                conflict_msg += "• 'Cancel': Cancel editing the tier"

                choice = messagebox.askyesnocancel(
                    "HS Range Conflict", conflict_msg
                )

                if choice is None:  # Cancel
                    return
                elif choice:  # Auto-adjust
                    if not self._auto_adjust_range_for_edit(
                        temp_tier, overlapping_tiers, index
                    ):
            ...
            if (
                original_min_hs != tier_to_edit.min_hs
                or original_max_hs != tier_to_edit.max_hs
            ):
    ...
    def _has_overlapping_ranges(self, new_tier: HandStrengthTier) -> bool:
        ...
        for existing_tier in self.strategy_data.tiers:
            ...
            if (
                new_tier.min_hs <= existing_tier.max_hs
                and new_tier.max_hs >= existing_tier.min_hs
            ):
                return True
        return False
    ...
    def _resolve_overlapping_ranges(self, new_tier: HandStrengthTier) -> bool:
        ...
        conflict_msg = f"HS range {
            new_tier.min_hs}-{
            new_tier.max_hs} overlaps with existing tiers:\n\n"
        for tier in overlapping_tiers:
            conflict_msg += f"• {tier.name} (HS {tier.min_hs}-{tier.max_hs})\n"

        conflict_msg += "\nChoose an option:\n"
        conflict_msg += (
            "• 'Auto-adjust': Automatically adjust the new tier's range\n"
        )
    ...
    def _auto_adjust_range(
        self,
        new_tier: HandStrengthTier,
        overlapping_tiers: List[HandStrengthTier],
    ) -> bool:
        """Automatically adjust the new tier's HS range to avoid overlaps."""
        # Find the largest gap in HS ranges
        all_ranges = [
            (tier.min_hs, tier.max_hs) for tier in self.strategy_data.tiers
        ]
        ...
        if gaps:
            ...
            messagebox.showinfo(
                "Range Adjusted",
                f"New tier range adjusted to HS {
                    new_tier.min_hs}-{
                    new_tier.max_hs}",
            )
            return True
    ...
    def _auto_adjust_range_for_edit(
        self,
        tier_to_edit: HandStrengthTier,
        overlapping_tiers: List[HandStrengthTier],
        edit_index: int,
    ) -> bool:
    ...
    def set_selected_hand(self, hand: str):
        ...
        self.selected_hand_label.config(
            text=f"Selected Hand: {hand} (Current HS: {current_hs})"
        )
        self.selected_hand_frame.pack(fill=tk.X, padx=5, pady=3)
    ...
    def _update_hs_score(self):
        ...
        try:
            ...
            if new_hs < 0 or new_hs > 100:
                messagebox.showerror(
                    "Invalid HS Score", "HS score must be between 0 and 100"
                )
                return
            ...
            if (
                "preflop"
                not in self.strategy_data.strategy_dict["hand_strength_tables"]
            ):
                self.strategy_data.strategy_dict["hand_strength_tables"][
                    "preflop"
                ] = {}
            ...
            # Update the display
            self.selected_hand_label.config(
                text=f"Selected Hand: {
                    self.selected_hand} (Current HS: {new_hs})"
            )
            ...
            messagebox.showinfo(
                "Success",
                f"HS score for {
                    self.selected_hand} updated to {new_hs}",
            )

        except ValueError:
            messagebox.showerror(
                "Invalid Input", "Please enter a valid number for HS score"
            )

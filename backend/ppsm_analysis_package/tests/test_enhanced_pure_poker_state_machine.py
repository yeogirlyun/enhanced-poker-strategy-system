...
class EnhancedPurePokerStateMachineTestSuite:
    ...
    def test_hand_startup_and_blinds(self):
        ...
        expected_pot = 0.0  # Blinds stay in current_bet until round completes
        self.log_test(
            "Initial Pot Calculation", 
            ppsm.game_state.pot == expected_pot,
            f"Pot: ${ppsm.game_state.pot} (correct: ${expected_pot} during betting)"
        )
    ...
    def test_action_execution(self):
        ...
        initial_pot = ppsm.game_state.pot
        ...
        # During active betting, pot should remain at 0 (bets in current_bet until round completes)
        self.log_test("Pot During Betting", ppsm.game_state.pot == 0, f"Pot correctly stays at ${ppsm.game_state.pot} during betting")
    ...
    def test_pot_and_stack_tracking(self):
        ...
        total_chips_before = sum(p.stack + p.current_bet for p in ppsm.game_state.players) + ppsm.game_state.pot
        ...
        total_chips_after = sum(p.stack + p.current_bet for p in ppsm.game_state.players) + ppsm.game_state.pot

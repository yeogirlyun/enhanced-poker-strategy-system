...
class PokerKitStrategyEngine:
    ...
    def evaluate_hand_strength(self, hole_cards: List[str], board: List[str]) -> Tuple[str, float]:
        """Evaluate hand strength using our hand evaluator."""
        hand_rank, kickers, strength = self.hand_evaluator.evaluate_hand(hole_cards, board)
        return hand_rank.name, strength
    
    def get_strategy_action(self, player: StrategyPlayer, game_state: StrategyGameState) -> StrategyAction:
        """Get strategy-based action for a player."""
        if not player.cards:
            return StrategyAction.FOLD
        
        # Evaluate hand strength
        hand_rank, strength = self.evaluate_hand_strength(player.cards, game_state.board)
    ...
    def _get_preflop_action(self, player: StrategyPlayer, game_state: StrategyGameState, strength: float) -> StrategyAction:
    ...
    def _get_postflop_action(self, player: StrategyPlayer, game_state: StrategyGameState, strength: float, hand_rank: str) -> StrategyAction:
    ...
    def execute_action(self, player: StrategyPlayer, action: StrategyAction, bet_size: float, game_state: StrategyGameState) -> None:
    ...
    def _log_deviation(self, player: StrategyPlayer, action: StrategyAction, game_state: StrategyGameState) -> None:
    ...
    def _determine_winner(self, game_state: StrategyGameState) -> StrategyPlayer:
        ...
        for player in active_players:
            strength = self.hand_evaluator.evaluate_hand(player.cards, game_state.board)[2]
    ...
    def get_session_report(self) -> Dict[str, Any]:
        ...
        correct_actions = len([d for d in self.deviation_logs if d["deviation_type"] == "correct"])

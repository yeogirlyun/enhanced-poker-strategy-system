...
class ProfessionalPokerTable:
    ...
    def _create_professional_controls(self, parent_frame):
        ...
        bet_entry = ttk.Entry(right_frame, textvariable=self.bet_size_var, width=8, font=("Arial", 10))
    ...
    def _draw_professional_table(self):
        ...
        for i in range(0, self.table_width, 25):
            for j in range(0, self.table_height, 25):
                pattern_x = table_x + i
                pattern_y = table_y + j
                if (pattern_x - self.center_x) ** 2 + (pattern_y - self.center_y) ** 2 < (self.player_radius - 30) ** 2:
                    self.canvas.create_oval(
                        pattern_x, pattern_y,
        ...
        # Draw professional pot area
        pot_radius = 35
        self.canvas.create_oval(
            self.center_x - pot_radius, self.center_y + 20,
            self.center_x + pot_radius, self.center_y + 90,
    ...
    def _draw_professional_player(self, player: Player, position: Tuple[int, int], is_current_player: bool = False):
    ...
    def _draw_professional_pot_info(self):
        ...
        if self.current_game_state.current_bet > 0:
            self.canvas.create_text(
                self.center_x, self.center_y + 75,

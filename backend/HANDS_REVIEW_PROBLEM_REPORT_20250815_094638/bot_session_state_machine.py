...
class HandsReviewBotSession(BotSessionStateMachine):
    ...
    def start_hand(self, existing_players: Optional[List[Player]] = None):
        ...
        if self.preloaded_hand_data:
            ...
            print(f"🃏 HANDS_REVIEW: Calling super().start_hand() - BotSessionStateMachine")
    ...
    def _start_hand_from_hand_model(self, hand_model: Hand):
        ...
        for seat in hand_model.seats:
            ...
            hole_cards = []
            if hasattr(hand_model.metadata, 'hole_cards') and seat.player_id in hand_model.metadata.hole_cards:
                hole_cards = hand_model.metadata.hole_cards[seat.player_id]
            
            player = Player(
                name=seat.player_id,  # Use canonical player ID (e.g., "Player1")
        ...
        print(f"🃏 HAND_MODEL: Session started successfully - pot: ${self.game_state.pot}, current_bet: ${self.game_state.current_bet}")

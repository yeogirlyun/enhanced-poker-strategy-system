...
class PokerStateMachine:
    ...
    def handle_start_hand(self):
        ...
        self.game_state = GameState(
            players=players, board=[], pot=1.5, current_bet=1.0, street="preflop"
        )
    ...
    def handle_deal_flop(self):
        ...
        self.action_player_index = (self.dealer_position + 1) % len(
            self.game_state.players
        )
        while not self.game_state.players[self.action_player_index].is_active:
            self.action_player_index = (self.action_player_index + 1) % len(
                self.game_state.players
            )
    ...
    def execute_bot_action(self, player: Player):
        ...
        if self.game_state.current_bet == 0:
            ...
            action = (
                ActionType.CALL
                if player.stack >= self.game_state.current_bet
    ...
    def execute_action(self, player: Player, action: ActionType, amount: float = 0):
        ...
        if action == ActionType.FOLD:
            ...
            call_amount = (
                amount if action == ActionType.BET else self.game_state.current_bet
            )
    ...
    def is_valid_action(
        self, player: Player, action: ActionType, amount: float = 0
    ) -> bool:
    ...
    def get_action_player(self) -> Optional[Player]:
        ...
        if self.game_state and 0 <= self.action_player_index < len(
            self.game_state.players
        ):

...
class PokerPracticeSimulator:
    ...
    def get_position_for_player(
        self, player_index: int, dealer_pos: int, num_players: int
    ) -> Position:
    ...
    def get_strategy_action(
        ...
        if game_state.street == "preflop":
            ...
            open_rules = self.strategy_data.strategy_dict.get("preflop", {}).get(
                "open_rules", {}
            )
    ...
    def log_deviation(
        ...
        deviation_log = DeviationLog(
            ...
            deviation_type=(
                "action_mismatch" if action_taken != strategy_action else "bet_size"
            ),
    ...
    def play_hand(self, num_players: int = 6) -> Dict[str, Any]:
        ...
        game_state = GameState(players=players, small_blind=1.0, big_blind=2.0)
        ...
        return {
            ...
            "deviations": len(
                [
                    log
                    for log in self.deviation_logs
                    if log.hand_id == f"hand_{self.session_stats['hands_played']-1}"
                ]
            ),
        }
    ...
    def play_street(self, game_state: GameState, street: str) -> None:
        ...
        while len(active_players) > 1:
            ...
            if current_player.is_human:
                strategy_action, strategy_bet = self.get_strategy_action(
                    current_player, game_state
                )
                if action != strategy_action:
                    self.log_deviation(
                        current_player, action, strategy_action, game_state
                    )
    ...
    def get_session_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "accuracy": (
                self.session_stats["correct_plays"]
                / max(1, self.session_stats["hands_played"])
            )
            * 100,
...
class PracticeSimulatorGUI:
    ...
    def create_widgets(self):
        ...
        config_frame = ttk.LabelFrame(
            self.root, text="Game Configuration", style="Dark.TLabelframe"
        )
        ...
        start_button = ttk.Button(
            config_frame, text="Start New Hand", command=self.start_new_hand
        )
        ...
        submit_button = ttk.Button(
            action_frame, text="Submit Action", command=self.submit_action
        )

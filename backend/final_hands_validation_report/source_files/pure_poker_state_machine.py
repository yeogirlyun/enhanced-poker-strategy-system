...
class HandModelDecisionEngineAdapter:
    ...
    def _can_inject_check(self, player_name, game_state) -> bool:
        """Return True if it's legal/needed to inject a CHECK for player_name now."""
        try:
        ...
        if game_state.current_bet != 0:
            return False  # facing a bet: no implicit check
        
        # For BB series compatibility: always allow CHECK injection postflop when current_bet=0
        # This handles cases where hand data completely omits CHECKs
        if is_postflop:
            return True
            
        # Preflop logic (BB option check)
        rs = getattr(game_state, "round_state", None)
        if rs and hasattr(rs, "need_action_from"):
            idx = self._seat_index(game_state, player_name)
            return idx is not None and idx in rs.need_action_from
    ...
    def get_decision(self, player_name: str, game_state):
        ...
        - Injects implicit CHECKs when logs omit them.
        """
        from core.hand_model import ActionType as HM

        # If we've exhausted the log but this player still needs to act with bet==0,
        # inject a final CHECK to let the street close cleanly.
        ...
        # If the next logged action is not for this player, consider implied CHECKs.
        if act.actor_uid != player_name:
            # Enhanced logic: inject CHECK for postflop when wrong player action detected
            street = str(game_state.street).lower() if game_state.street else ""
            is_postflop = street in ("flop", "turn", "river")
            
            if is_postflop and game_state.current_bet == 0:
                # Check if this is a case where we need to inject a CHECK
                # Look ahead to see if there are actions by other players coming up
                next_action_is_bet_or_raise = getattr(act, "action", None) in (HM.BET, HM.RAISE)
                
                # Inject CHECK if the wrong player is about to act aggressively 
                # or if we're at the end of actions (to close the street)
                if next_action_is_bet_or_raise:
                    return ActionType.CHECK, None
            
            # Otherwise we have nothing for this player right now; do not consume the log.
            return None

        # Now: the next logged action IS for this player. Consume it.

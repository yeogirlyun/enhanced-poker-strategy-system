...
class CLIPokerGame:
    ...
    def _display_game_state(self):
        ...
        for i, player in enumerate(self.current_game_state.players):
            ...
            bet_info = (
                f" (Bet: ${player.current_bet:.2f})" if player.current_bet > 0 else ""
            )

            print(
                f"{player.name} ({player.position}): ${player.stack:.2f} - {status}{bet_info}{turn_indicator}"
            )
    ...
    def start_hand(self):
        ...
        random.shuffle(deck)
        print(f"DEBUG: Deck has {len(deck)} cards")
        
        # Start the state machine with cards
        self.state_machine.start_hand_with_cards(deck)
        self.current_game_state = self.state_machine.game_state
        
        print(f"DEBUG: After dealing, deck has {len(deck)} cards")
        print(f"DEBUG: Player 1 cards: {self.current_game_state.players[0].cards}")
        print(f"DEBUG: All players cards:")
        for i, player in enumerate(self.current_game_state.players):
            print(f"  Player {i+1}: {player.cards}")

        # Display initial state AFTER cards are dealt
        self._display_game_state()
    ...
    def run_game(self, num_hands=3):
        ...
        print("=" * 60)
        print(
            f"Strategy: {self.strategy_data.strategy_file if hasattr(self.strategy_data, 'strategy_file') else 'modern_strategy.json'}"
        )

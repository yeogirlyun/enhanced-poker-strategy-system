...
class SoundTesterApp:
    ...
    def setup_gui(self):
        ...
        title_label = ttk.Label(
            main_frame,
            text="🎵 Poker Sound Tester & Configuration Tool",
            font=("Arial", 16, "bold"),
        )
        ...
        files_frame = ttk.LabelFrame(
            main_frame, text="Available Sound Files", padding="10"
        )
        files_frame.grid(
        ...
        self.files_listbox = tk.Listbox(
            files_list_frame, height=20, selectmode=tk.EXTENDED
        )
        ...
        play_file_btn = ttk.Button(
            top_buttons, text="🔊 Play", command=self.play_selected_file
        )
        play_file_btn.pack(side=tk.LEFT, padx=(0, 5))

        # Delete button for selected file
        delete_file_btn = ttk.Button(
            top_buttons, text="🗑️ Delete", command=self.delete_selected_file
        )
        delete_file_btn.pack(side=tk.LEFT, padx=5)

        # File info button
        info_btn = ttk.Button(
            top_buttons, text="ℹ️ Info", command=self.show_file_info
        )
        info_btn.pack(side=tk.LEFT, padx=5)

        # Refresh button
        refresh_btn = ttk.Button(
            top_buttons, text="🔄 Refresh", command=self.refresh_sound_files
        )
        ...
        # Bulk delete button
        bulk_delete_btn = ttk.Button(
            bottom_buttons,
            text="🗑️ Delete Multiple",
        ...
        select_all_btn = ttk.Button(
            bottom_buttons, text="☑️ Select All", command=self.select_all_files
        )
        select_all_btn.pack(side=tk.LEFT, padx=5)

        # Right panel - Event Configuration
        config_frame = ttk.LabelFrame(
            main_frame, text="Poker Event Sound Configuration", padding="10"
        )
        ...
        self.events_tree = ttk.Treeview(
            config_frame, columns=("current_sound",), height=15
        )
        ...
        events_scrollbar = ttk.Scrollbar(
            config_frame, orient=tk.VERTICAL, command=self.events_tree.yview
        )
        ...
        play_event_btn = ttk.Button(
            event_buttons_frame,
            text="🔊 Play Event Sound",
        ...
        assign_btn = ttk.Button(
            event_buttons_frame,
            text="📝 Assign Selected File to Event",
        ...
        config_mgmt_frame = ttk.LabelFrame(
            bottom_frame, text="Configuration Management", padding="10"
        )
        ...
        self.status_var.set(
            "Ready - Select a sound file and poker event to configure"
        )
    ...
    def refresh_sound_files(self):
        ...
        for file_path in sounds_dir.rglob("*"):
            if (
                file_path.is_file()
                and file_path.suffix.lower() in sound_extensions
            ):
    ...
    def populate_events_tree(self):
        ...
        for event_id, event_info in self.poker_events.items():
            current_sound = self.sound_config.get(
                event_id, event_info["current"]
            )
    ...
    def play_selected_file(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection", "Please select a sound file to play"
            )
            return
    ...
    def delete_selected_file(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection", "Please select a sound file to delete"
            )
            return
        ...
        try:
            ...
            if file_in_use:
                ...
                messagebox.showinfo(
                    "File Deleted", f"File deleted successfully: {file_path}"
                )
    ...
    def show_file_info(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection", "Please select a sound file to view info"
            )
            return
    ...
    def bulk_delete_files(self):
        ...
        if not selections:
            messagebox.showwarning(
                "No Selection",
                "Please select one or more sound files to delete",
            )
            return
            ...
            try:
                ...
                for event_id, event_info in self.poker_events.items():
                    ...
                    if (
                        ...
                        and event_id in self.sound_config
                    ):
                        del self.sound_config[event_id]
        ...
        # Show results
        result_msg = "Bulk deletion completed:\n\n"
        ...
        self.status_var.set(
            f"Bulk delete: {deleted_count} deleted, {
                len(errors)} errors"
        )
    ...
    def play_event_sound(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection", "Please select a poker event"
            )
            return

        event_id = selection[0]
        sound_file = self.sound_config.get(
            event_id, self.poker_events[event_id]["current"]
        )
    ...
    def assign_sound_to_event(self):
        ...
        if not file_selection:
            messagebox.showwarning(
                "No File Selected", "Please select a sound file first"
            )
            return
        ...
        if not event_selection:
            messagebox.showwarning(
                "No Event Selected", "Please select a poker event"
            )
            return
    ...
    def save_config(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Saved",
                f"Sound configuration saved to {
                    self.config_file}",
            )

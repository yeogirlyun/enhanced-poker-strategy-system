...
class GameConfig:
    ...
    num_players: int = 6
    big_blind: float = (
        2.0  # Changed from 1.0 to 2.0 (no floating point)
    )
    small_blind: float = (
        1.0  # Changed from 0.5 to 1.0 (no floating point)
    )
    ...
    def __post_init__(self):
        ...
        if self.big_blind <= 0:
            raise ValueError(
                f"big_blind must be positive, got {
                    self.big_blind}"
            )
        if self.small_blind <= 0:
            raise ValueError(
                f"small_blind must be positive, got {
                    self.small_blind}"
            )
...
class FlexiblePokerStateMachine:
    ...
    def __init__(
        self, config: GameConfig = None, mode: str = "live"
    ):
        ...
        self.strategy_integration = (
            self._create_basic_strategy_integration()
        )
    ...
    def restore_snapshot(self, snap: Dict[str, Any]) -> None:
        ...
        print(
            f"   🎲 Dealer: {snap.get('dealer_index', 'Not set')}"
        )
        action_player = snap.get(
            "action_player_index", "Not set"
        )
        ...
        try:
            ...
            self.game_state.street = snap.get(
                "street", "preflop"
            )
            self.game_state.current_bet = float(
                snap.get("current_bet", 0.0)
            )
            ...
            for i, p in enumerate(players):
                if i < len(self.game_state.players):
                    ...
                    gp.current_bet = float(
                        p.get("current_bet", 0.0)
                    )
            ...
            if "action_player_index" in snap:
                self.action_player_index = int(
                    snap["action_player_index"]
                )
                print(
                    f"   🎯 Action player: {self.action_player_index}"
                )
        ...
        for i in range(num_players):
            ...
            if self.config.num_players == 2:
                ...
                if i == self.dealer_position:
                    self.game_state.players[i].position = (
                        "SB/BTN"
                    )
                else:
                ...
                position_offset = (
                    i - self.dealer_position
                ) % num_players

                if position_offset < len(position_names):
                    self.game_state.players[i].position = (
                        position_names[position_offset]
                    )
                else:
                    self.game_state.players[i].position = (
                        f"Seat{i + 1}"
    ...
    def _get_position_names(self) -> List[str]:
        ...
        if num_players == 2:
            ...
            # 6-handed: [BTN, SB, BB, UTG, MP, CO] (starting from dealer
            # position)
    ...
    def start_hand(
        ...
        # Post blinds
        sb_player = self.game_state.players[
            self.small_blind_position
        ]
        bb_player = self.game_state.players[
            self.big_blind_position
        ]
    ...
    def transition_to(self, new_state: PokerState):
        ...
        valid_transitions = self.STATE_TRANSITIONS.get(
            self.current_state, []
        )
        if (
        ...
        if new_state == PokerState.DEAL_FLOP:
            ...
            if winners:
                pot_per_winner = self.game_state.pot / len(
                    winners
                )
            ...
            primary_winner_name = (
                winners[0].name if winners else "Unknown"
            )
            winner_info = {
            ...
            if winners and hasattr(self, "hand_evaluator"):
                try:
                    ...
                    if (
                        first_winner.cards
                        and len(first_winner.cards) == 2
                    ):
                        hand_eval = (
                            self.hand_evaluator.evaluate_hand(
                                first_winner.cards,
                                self.game_state.board,
                            )
                        )
    ...
    def _find_first_active_after_dealer(self) -> int:
        ...
        start = (self.dealer_position + 1) % len(
            self.game_state.players
        )
    ...
    def execute_action(
        ...
        if self.current_state not in betting_states:
            self._safe_print(
                f"❌ Action rejected: {
                    action.value} not allowed during {
                    self.current_state}"
            )
            return False
        ...
        if (
            ...
            and self.session_logger
        ):
            self.session_logger.log_system(
                "DEBUG",
                "GAME_STATE",
                f"Action executed: {
                    self.current_state}, Street: {
        ...
        if action == ActionType.FOLD:
            ...
            if call_amount > 0:
                ...
                player.current_bet = round(
                    player.current_bet + actual_call, 2
                )
                player.stack = round(
                    player.stack - actual_call, 2
                )
                self.game_state.pot = round(
                    self.game_state.pot + actual_call, 2
                )
                self._safe_print(
                    f"   {player.name} calls ${actual_call:.2f}"
                )
            ...
            if amount > 0:
                ...
                self.game_state.current_bet = player.current_bet
                self._safe_print(
                    f"   {player.name} bets ${actual_amount:.2f}"
                )
        ...
        if (
            ...
            self.session_logger.log_system(
                ...
                f"Round progress: {
                    ...
                    len(
                        self.players_acted_this_round)} players acted",
                {
        ...
        if (
            ...
            self.session_logger.log_system(
                ...
                f"After {
                    player.name}'s {
                    action.value}: checking round completion",
                {
                    "player": player.name,
                    "action": action.value,
                    "amount": amount,
    ...
    def _advance_action_player(self):
        ...
        next_index = self._find_next_active_player(
            self.action_player_index
        )
        if next_index == -1:
            self.transition_to(PokerState.END_HAND)
        ...
        self._emit_event(
            GameEvent(
                ...
                timestamp=time.time(),
                player_name=self.game_state.players[
                    next_index
                ].name,
            )
        )
    ...
    def _is_round_complete(self) -> bool:
        ...
        all_equal = all(
            (p.current_bet == max_bet) or p.is_all_in
            for p in active_players
        )
        ...
        # When equalized, require a full pass of actions among players who can
        # act
        required_actions = max(1, len(actionable_players))

        # If no bet (max_bet == 0), ensure every actionable player has acted
        # once
        if max_bet == 0:
    ...
    def _handle_round_complete(self):
        ...
        if len(active_players) <= 1:
            self._safe_print(
                f"🏁 Only {
                    len(active_players)} active players, ending hand"
            )
            self.transition_to(PokerState.END_HAND)
            return
    ...
    def determine_winners(
            ...
            if len(player.cards) == 2:
                ...
                self._safe_print(
                    f"🃏 {
                        player.name}: {
                        player.cards} + {
                        self.game_state.board} = {desc} (score={score})"
                )
        ...
        winners_with_evals = (
            self.hand_evaluator.determine_winners(
                player_evaluations
            )
        )
        winners = [
            player for player, eval_data in winners_with_evals
        ]

        # Debug logging for final winner determination
        if len(winners) == 1:
            winner_eval = winners_with_evals[0][1]
            hand_desc = winner_eval.get(
                "hand_description", "Unknown"
            )
            best_five = winner_eval.get("best_five_cards", [])
            five_cards_str = (
                f" [{', '.join(best_five)}]" if best_five else ""
            )
            self._safe_print(
                f"🏆 Single winner: {
                    winners[0].name} with {hand_desc}{five_cards_str}"
            )
        else:
            ...
            self._safe_print(
                f"🤝 Split pot: {
                    ', '.join(winner_names)} tie with {hand_desc}{five_cards_str}"
            )
    ...
    def get_action_player(self) -> Optional[Player]:
        ...
        try:
            if (
                not self.game_state
                or not self.game_state.players
            ):
                return None
            ...
            if (
                ...
                if hasattr(player, "is_human"):
                    ...
                    self.session_logger.log_system(
                        "ERROR",
                        "STATE_MACHINE",
                        f"get_action_player returning non-Player object: {
                            type(player)} - {player}",
                        {
    ...
    def get_valid_actions_for_player(
        self, player: Player
    ) -> Dict[str, Any]:
    ...
    def set_player_cards(
        ...
        if 0 <= player_index < len(self.game_state.players):
            self.game_state.players[player_index].cards = (
                cards.copy()
            )
    ...
    def set_player_folded(self, player_index: int, folded: bool):
        ...
        if 0 <= player_index < len(self.game_state.players):
            self.game_state.players[player_index].has_folded = (
                folded
            )
            self.game_state.players[player_index].is_active = (
                not folded
            )
    ...
    def _create_basic_strategy_integration(self):
        ...
        class BasicStrategyIntegration:
            ...
            def get_action(
                self, player, game_state, valid_actions
            ):

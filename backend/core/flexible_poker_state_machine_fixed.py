    ...
    def start_hand(
        ...
        if existing_players:
            ...
            if (
                ...
                raise ValueError(
                    f"Player count mismatch: config expects {self.config.num_players}, got {len( self.game_state.players)}"
                )
        else:
            # Reset player states
    ...
    def execute_action(
        ...
        if (
            ...
            self.session_logger.log_system(
                "DEBUG",
                "ROUND_PROGRESS",
                f"Round progress: {self.actions_this_round} actions, {len( self.players_acted_this_round)} players acted",
                {

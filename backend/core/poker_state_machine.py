...
class ImprovedPokerStateMachine:
    ...
    def get_basic_bot_action(self, player: Player) -> Tuple[ActionType, float]:
        """Get bot action using GTO strategy or custom strategy file."""
        # Default to GTO strategy unless custom strategy is specified
        ...
        elif hasattr(self, 'custom_strategy_data') and self.custom_strategy_data:
            # TODO: Implement custom strategy file support
            # For now, fall back to GTO
            return self.strategy_engine.get_gto_bot_action(player, self.game_state)
        else:
            # Default to GTO strategy
            return self.strategy_engine.get_gto_bot_action(player, self.game_state)
    ...
    def validate_action(self, player: Player, action: ActionType, amount: float = 0) -> List[str]:
        ...
        # NEW: Stack limit check
        effective_amount = amount - player.current_bet if action in [ActionType.RAISE, ActionType.BET] else amount
        if effective_amount > player.stack:
            errors.append(f"Amount ${effective_amount:.2f} exceeds stack ${player.stack:.2f}")

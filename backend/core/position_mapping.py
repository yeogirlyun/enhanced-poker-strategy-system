...
class PositionMapper:
    ...
    def _build_position_index(self):
        ...
        self.position_to_index = {
            pos: idx for idx, pos in enumerate(self.positions)
        }
        self.index_to_position = {
            idx: pos for idx, pos in enumerate(self.positions)
        }

    def get_position_for_seat(
        self, seat_index: int, dealer_position: int
    ) -> str:
    ...
    def map_strategy_position(
        self, actual_position: str, strategy_positions: List[str]
    ) -> Optional[str]:
        """
    ...
    def get_relative_position(self, position: str) -> float:
        ...
        non_blind_positions = [
            p for p in self.positions if p not in ["SB", "BB"]
        ]
            ...
            if action_type in postflop:
                for street in ["flop", "turn", "river"]:
                    if street in postflop[action_type]:
                        self.available_positions.update(
                            postflop[action_type][street].keys()
                        )

    def get_strategy_for_position(
        self, actual_position: str, street: str, action_context: str = "open"
    ) -> Dict:
        """
        ...
        if street == "preflop":
            if action_context == "open":
                return self.strategy_data["preflop"]["open_rules"].get(
                    mapped_position,
                    self._get_default_strategy(street, action_context),
                )
            else:  # vs_raise
...
class HandHistoryManager:
    ...
    def log_action(self, player, action, amount: float, game_state):
        ...
        if (
            self.test_mode
            and hasattr(action, "value")
            and action.value == "raise"
        ):

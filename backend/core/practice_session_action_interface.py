...
from .practice_session_poker_state_machine import PracticeSessionPokerStateMachine
...
class PracticeSessionActionInterface:
    ...
    def __init__(self, state_machine: PracticeSessionPokerStateMachine, game_director=None):
    ...
    def can_human_act(self) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error checking human action: {e}")
            return False
    ...
    def get_available_actions(self) -> List[str]:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error getting available actions: {e}")
    ...
    def get_action_button_text(self, action_key: str) -> str:
        ...
        try:
            if action_key == 'fold':
                ...
                return "Check" if call_amount == 0 else f"Call ${call_amount:.0f}"
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error getting button text: {e}")
            return action_key.title()
    ...
    def execute_check_call(self) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error executing check/call: {e}")
            return False
    ...
    def execute_fold(self) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error executing fold: {e}")
            return False
    ...
    def execute_bet_raise(self, amount: Optional[float] = None) -> bool:
        ...
        try:
            ...
            action_type = ActionType.BET if current_bet == 0 else ActionType.RAISE
            ...
            if not self._is_valid_bet_amount(amount):
                if self.logger:
                    self.logger.log_system("WARNING", "ACTION_INTERFACE", f"Invalid bet amount: ${amount}")
                return False
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error executing bet/raise: {e}")
            return False
    ...
    def get_quick_bet_amount(self, bet_type: str) -> float:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error calculating quick bet: {e}")
    ...
    def execute_quick_bet(self, bet_type: str) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error executing quick bet: {e}")
            return False
    ...
    def can_execute_action(self, action_key: str) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error checking action availability: {e}")
            return False
    ...
    def get_board_info(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error getting board info: {e}")
            return None
    ...
    def _is_valid_action_state(self) -> bool:
        ...
        current_state_name = str(self.state_machine.current_state).split('.')[-1]
    ...
    def _calculate_min_raise_amount(self) -> float:
        ...
        try:
            ...
            return max(big_blind, current_bet * 2) if current_bet > 0 else big_blind
    ...
    def _execute_action(self, player, action_type: ActionType, amount: float) -> bool:
        """Execute an action through the GameDirector or state machine."""
        try:
            # Log the action attempt
            if self.logger:
                self.logger.log_system("INFO", "ACTION_INTERFACE", 
                                     f"Executing {action_type.value} for {player.name}", 
                                     {"amount": amount})
            
            # Use GameDirector if available (for sound coordination), otherwise fallback to state machine
            if self.game_director:
                success = self.game_director.execute_player_action(player, action_type, amount)
            else:
                success = self.state_machine.execute_action(player, action_type, amount)
            
            if success and self.logger:
                self.logger.log_system("INFO", "USER_ACTION", 
                                     f"Action completed: {action_type.value}", 
            ...
            if self.logger:
                self.logger.log_system("ERROR", "ACTION_INTERFACE", f"Error executing action: {e}")
            return False

    ...
    def __init__(self, config: GameConfig, strategy_data=None):
        ...
        try:
            ...
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "PracticeSessionPokerStateMachine initialization started",
                {
        ...
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "Calling parent FlexiblePokerStateMachine constructor",
                {},
            )
        super().__init__(config)

        # DISABLE auto_advance for practice session to maintain manual control
        # Practice session should have human interaction and manual progression
        # Auto-advance causes "two steps at a time" issues
        self.config.auto_advance = False  # Manual control for practice session
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "Auto_advance disabled for practice session (manual control)",
                {"auto_advance": False},
            )
        ...
        if self.logger:
            self.logger.log_system(
                "INFO", "PRACTICE_SM_INIT", "Creating ImprovedGTOStrategy", {}
            )
        try:
            from .improved_gto_strategy import ImprovedGTOStrategy

            self.improved_gto_strategy = ImprovedGTOStrategy(
                self.config.num_players
            )
            # Keep old strategy engine as fallback
            self.strategy_engine = GTOStrategyEngine(
                self.config.num_players, strategy_data
            )
            if self.logger:
                self.logger.log_system(
                    "INFO",
                    "PRACTICE_SM_INIT",
                    "ImprovedGTOStrategy created successfully",
                    {},
                )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_SM_INIT",
                    f"Failed to create ImprovedGTOStrategy: {e}",
        ...
        if self.game_state.players:
            ...
            if self.logger:
                self.logger.log_system(
                    ...
                    "PRACTICE_SM_INIT",
                    f"Marked {
                        self.game_state.players[0].name} as human player",
                    {
            ...
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "PRACTICE_SM_INIT",
                    "No players found in game_state",
                    {},
                )
        ...
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "PracticeSessionPokerStateMachine initialization completed successfully",
                {},
            )
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "Practice Session State Machine ready",
                {},
            )
    ...
    def calculate_quick_bet_amount(self, bet_type: str) -> float:
        ...
        try:
            ...
            try:
                snapshot = (
                    self.get_game_info()
                    if hasattr(self, "get_game_info")
                    else None
                )
                snapshot_pot = (
                    float(snapshot.get("pot", 0.0)) if snapshot else 0.0
                )
            except Exception:
                snapshot_pot = 0.0
            ...
            current_pot = max(
                0.0, snapshot_pot if snapshot_pot > 0 else gs_pot
            )
            ...
            player_bet = (
                getattr(human_player, "current_bet", 0.0)
                if human_player
                else 0.0
            )
            call_amount = max(0.0, current_bet - player_bet)
            min_raise = getattr(
                self.game_state, "min_raise", self.config.big_blind
            )
            ...
            def _target_amount_from_fraction(frac: float) -> float:
                ...
                # If facing a bet → raise-to current_bet + call + ceil(pot *
                # frac)
                if call_amount <= 0.0:
                    return _round_up(current_pot * frac)
                # Raise-to target
                raise_to = (
                    current_bet + call_amount + _round_up(current_pot * frac)
                )
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_SM",
                    f"Error calculating quick bet amount: {e}",
                    {},
                )
            return 10.0  # Safe fallback

    def get_human_player(self):
        """Get the human player from the game state."""
        try:
            return next(
                (p for p in self.game_state.players if p.is_human), None
            )
        except Exception:
            return None

    def get_current_analysis(self) -> str:
        """Get analysis of current hand situation (encapsulates analysis logic)."""
        try:
            if self.hands_played <= 0:
                return "📊 Start a hand first to get analysis."

            stats = self.get_practice_stats()
            human_player = self.get_human_player()
            position = human_player.position if human_player else "Unknown"

            return f"""📊 Current Hand Analysis:
...
Pot: ${self.game_state.pot:.2f}"""
            ...
            if self.logger:
                self.logger.log_system(
                    "ERROR",
                    "PRACTICE_SM",
                    f"Error getting current analysis: {e}",
                    {},
                )
    ...
    def start_hand(self, existing_players: List[Player] = None):
        ...
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_HAND",
                "Starting practice hand with educational features",
                {},
            )
    ...
    def execute_action(
        self, player: Player, action_type: ActionType, amount: float = 0.0
    ) -> bool:
        ...
        if success:
            ...
            # FIXED: Only include actual betting states, not dealing states
            betting_states = [
            ...
            if self.current_state in betting_states:
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "BOT_SCHEDULING",
                        f"Scheduling bot actions after {
                            player.name}'s {
                            action_type.value} (state: {
                            self.current_state})",
                        {
                            "player": player.name,
                ...
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "BOT_SCHEDULING",
                        f"Not scheduling: Current state {
                            self.current_state} not a betting state",
                        {"state": self.current_state.name},
                    )
    ...
    def _advance_action_player(self):
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "ACTION_ADVANCE",
                f"Action player advanced: {old_action_player} → {new_action_player}",
                {"old": old_action_player, "new": new_action_player},
            )

        # GameDirector handles all timing - no need for manual timer management

        # Schedule bot actions after action player advancement (betting streets
        # only)
        betting_states = [
            PokerState.PREFLOP_BETTING,
            PokerState.FLOP_BETTING,
        ...
        if (
            self.current_state in betting_states
            and new_action_player >= 0
            and new_action_player < len(self.game_state.players)
        ):
            next_player = self.game_state.players[new_action_player]
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "PRACTICE_ACTION_ADVANCE",
                    f"[PRACTICE_PSM] Next player: {
                        next_player.name} (is_human={
                        next_player.is_human})",
                    {
                        "player": next_player.name,
                        "is_human": next_player.is_human,
                    },
                )
            if not next_player.is_human:
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "PRACTICE_BOT_SCHEDULING",
                        f"[PRACTICE_PSM] Bot turn: {
                            next_player.name} - GameDirector will handle timing",
                        {"player": next_player.name},
                    )
                ...
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "PRACTICE_HUMAN_TURN",
                        f"[PRACTICE_PSM] Human turn: {
                            next_player.name} - waiting for user input",
                        {"player": next_player.name},
                    )
    ...
    def transition_to(self, new_state: PokerState):
        ...
        if self.logger:
            self.logger.log_system(
                "DEBUG",
                "STATE_TRANSITION",
                f"Practice state transition: {
                    old_state.name} → {
                    new_state.name}",
                {"old_state": old_state.name, "new_state": new_state.name},
            )

        # Handle bot auto-play after state transitions (betting streets only)
        betting_states = [
            PokerState.PREFLOP_BETTING,
            PokerState.FLOP_BETTING,
            PokerState.TURN_BETTING,
        ...
        if new_state in betting_states:
            ...
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "BOT_SCHEDULING",
                    f"Not scheduling: Transition to {new_state} - not a betting state",
                    {"new_state": new_state.name},
                )
    ...
    def _get_human_player_position(self) -> Optional[str]:
        ...
        for i, player in enumerate(self.game_state.players):
            if player.is_human:
                ...
                position_offset = (i - dealer_pos) % len(
                    self.game_state.players
                )

                if len(self.game_state.players) == 2:
                    return (
                        "Small Blind" if position_offset == 1 else "Big Blind"
                    )
    ...
    def _analyze_decision_quality(
        self, player: Player, action_type: ActionType, amount: float
    ) -> bool:
        ...
        if (
            ...
            and action_type == ActionType.FOLD
            and player.cards
        ):
    ...
    def _generate_action_feedback(
        ...
        if action_type == ActionType.FOLD:
            feedback += (
                " Remember to consider pot odds and your hand strength."
            )
    ...
    def _provide_street_analysis(self, street_state: PokerState):
        ...
        if street_state == PokerState.DEAL_FLOP:
            analysis += (
                "Consider how the flop connects with your hand and position."
            )
        elif street_state == PokerState.DEAL_TURN:
            analysis += (
                "The turn card changes pot odds and drawing possibilities."
            )
        elif street_state == PokerState.DEAL_RIVER:
            analysis += (
                "Final betting round - consider your final hand strength."
            )
    ...
    def get_practice_stats(self) -> Dict[str, Any]:
        ...
        win_rate = (
            self.session_stats["hands_won"] / max(self.hands_played, 1)
        ) * 100
        decision_accuracy = (
            self.correct_decisions / max(self.total_decisions, 1)
        ) * 100
    ...
    def reset_practice_stats(self):
        ...
        if self.logger:
            self.logger.log_system(
                "INFO", "SESSION", "Practice session statistics reset", {}
            )
        ...
        self._emit_event(
            GameEvent(
                ...
                timestamp=datetime.now(),
                data={
                    "message": "Practice session statistics have been reset"
                },
            )
        )
    ...
    def get_strategy_suggestion(self, player: Player) -> Optional[str]:
        ...
        if self.current_state == PokerState.PREFLOP_BETTING:
            suggestion += (
                "consider your hand strength and position for optimal play."
            )
        else:
            suggestion += (
                "evaluate your hand strength against the board texture."
            )
    ...
    def get_display_state(self) -> dict:
        ...
        # Card visibilities - in practice mode, human player cards should be
        # visible
        display_state["card_visibilities"] = []
        for i, player in enumerate(self.game_state.players):
            # Human player cards always visible, others based on game state
            is_visible = player.is_human or self.current_state in [
                PokerState.SHOWDOWN,
                PokerState.END_HAND,
            ]
        ...
        for i, player in enumerate(self.game_state.players):
            # Simple chip symbol
            display_state["chip_representations"][f"player{i}_stack"] = "🟡"
    ...
    def set_game_director(self, game_director):
        ...
        if self.logger:
            self.logger.log_system(
                "INFO",
                "PRACTICE_SM_INIT",
                "GameDirector set for state machine",
                {},
            )
        ...
        if self.current_state in [PokerState.END_HAND, PokerState.SHOWDOWN]:
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "PRACTICE_BOT_ACTION",
                    f"[PRACTICE_PSM] Not executing: Hand is ending (state: {
                        self.current_state})",
                    {"state": self.current_state.name},
                )
            return

        if self.action_player_index >= 0 and self.action_player_index < len(
            self.game_state.players
        ):

            current_player = self.game_state.players[self.action_player_index]

            # CRITICAL: Double-check that this is NOT a human player
            if current_player.is_human:
                if self.logger:
                    self.logger.log_system(
                        "ERROR",
                        "PRACTICE_BOT_ACTION",
                        f"[PRACTICE_PSM] CRITICAL PROTECTION: Bot execution attempted for HUMAN player {
                            current_player.name}!",
                        {
                            "player": current_player.name,
                            "action_index": self.action_player_index,
                        },
                    )
                ...
                if self.logger:
                    self.logger.log_system(
                        "ERROR",
                        "PRACTICE_BOT_ACTION",
                        "CRITICAL HUMAN PROTECTION stack",
                        {"stack": stack_trace},
                    )
                return
            ...
            if (
                not current_player.is_human
                and current_player.is_active
                and not current_player.has_folded
            ):
                ...
                if self.logger:
                    self.logger.log_system(
                        "DEBUG",
                        "PRACTICE_BOT_ACTION",
                        f"[PRACTICE_PSM] Bot execution check: {
                            current_player.name} ready to act",
                        {
                            "player": current_player.name,
                ...
                try:
                    if self.logger:
                        self.logger.log_system(
                            "DEBUG",
                            "PRACTICE_BOT_ACTION",
                            f"[PRACTICE_PSM] Getting strategy decision for {
                                current_player.name}",
                            {"player": current_player.name},
                        )
                    action, amount = self._get_bot_strategy_decision(
                        current_player
                    )
                    if self.logger:
                        self.logger.log_system(
                            "DEBUG",
                            "PRACTICE_BOT_ACTION",
                            f"[PRACTICE_PSM] Strategy decision complete: {
                                action.value} ${
                                amount:.2f}",
                            {
                                "player": current_player.name,
                    ...
                    if self.logger:
                        self.logger.log_system(
                            "ERROR",
                            "PRACTICE_BOT_ACTION",
                            f"[PRACTICE_PSM] Error getting strategy decision for {
                                current_player.name}: {e}",
                            {"player": current_player.name, "error": str(e)},
                        )
                    # Fallback to fold
                ...
                if self.logger:
                    self.logger.log_system(
                        ...
                        "PRACTICE_BOT_ACTION",
                        f"[PRACTICE_PSM] Bot {
                            current_player.name} auto-playing: {
                            action.value} ${
                            amount:.2f}",
                        {
                            "player": current_player.name,
                ...
                if success:
                    if self.logger:
                        self.logger.log_system(
                            "DEBUG",
                            "PRACTICE_BOT_ACTION",
                            "[PRACTICE_PSM] Bot action successful, checking for next bot",
                            {
                                "current_state": self.current_state.name,
                                "action_player_index": self.action_player_index,
                                "scheduled_flag": getattr(
                    ...
                    if self.logger:
                        self.logger.log_system(
                            "ERROR",
                            "PRACTICE_BOT_ACTION",
                            f"[PRACTICE_PSM] Bot action failed: {
                                action.value}",
                            {
                                "action": action.value,
                                "player": current_player.name,
            ...
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "PRACTICE_BOT_ACTION",
                    f"[PRACTICE_PSM] Invalid action_player_index: {
    ...
    def _get_bot_strategy_decision(
        ...
        # Maximum 8 actions per round (more aggressive)
        if self.actions_this_round >= 8:
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "PRACTICE_BOT_ACTION",
                    f"Action limit reached this round ({
                        self.actions_this_round}), forcing conservative play",
                    {},
                )
            ...
            if call_amount <= 0:
                ...
                call_amount <= bot_player.stack * 0.1
            ):  # Only call small amounts
                return ActionType.CALL, call_amount
            else:
                return ActionType.FOLD, 0.0

        # Use the improved GTO strategy engine first
        try:
            action, amount = self.improved_gto_strategy.get_gto_action(
                bot_player, self.game_state
            )
            if self.logger:
                self.logger.log_system(
                    "DEBUG",
                    "IMPROVED_GTO_STRATEGY",
                    f"Improved GTO Strategy: {
                        bot_player.name} -> {
                        action.value} ${
                        amount:.2f}",
                    {
            ...
            if self.logger:
                self.logger.log_system(
                    "WARNING",
                    "IMPROVED_GTO_STRATEGY",
                    f"Improved strategy error for {
                        bot_player.name}: {e}, trying fallback",
            ...
            try:
                ...
                if self.logger:
                    self.logger.log_system(
                        "ERROR",
                        "GTO_STRATEGY",
                        f"All strategies failed for {
                            bot_player.name}: {e2}, using simple fallback",
                        {"player": bot_player.name, "error": str(e2)},
                    )
                return self._get_simple_gto_decision(bot_player)

    def _get_simple_gto_decision(
        self, bot_player: Player
    ) -> tuple[ActionType, float]:
        """Improved fallback decision making when GTO engine fails."""
        ...
        # More realistic fallback logic - bots should play reasonably, not
        # ultra-tight
        if call_amount == 0:
            # No bet to call - can check or bet
            if valid_actions.get("check", False):
                # Sometimes bet for value (30% of the time)
                if random.random() < 0.3 and valid_actions.get("bet", False):
                    bet_amount = min(
                        self.game_state.pot * 0.5, bot_player.stack * 0.2
                    )
            ...
            # Facing a bet - call reasonable amounts, fold only when call is
            # large
            call_threshold = min(
                bot_player.stack * 0.3, self.game_state.pot * 2.0
            )  # Much more reasonable

            if call_amount <= call_threshold and valid_actions.get(
                "call", False
            ):
                return ActionType.CALL, call_amount
            else:

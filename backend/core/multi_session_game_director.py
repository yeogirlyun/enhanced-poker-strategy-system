...
class MultiSessionGameDirector:
    ...
    def __init__(self, session_logger: SessionLogger):
        ...
        self.session_logger.log_system(
            "INFO",
            "MULTI_GAME_DIRECTOR",
            "Multi-Session GameDirector initialized",
            {},
        )
    ...
    def register_session(
        ...
        self.session_logger.log_system(
            "INFO",
            "MULTI_GAME_DIRECTOR",
            f"Registered {session_type} session",
            {
    ...
    def start(self):
        ...
        self.session_logger.log_system(
            "INFO",
            "MULTI_GAME_DIRECTOR",
            "Multi-Session GameDirector started",
    ...
    def stop(self):
        ...
        self.session_logger.log_system(
            "INFO",
            "MULTI_GAME_DIRECTOR",
            "Multi-Session GameDirector stopped",
        ...
        def update_callback():
            if self.is_running:
                ...
                if active_session and active_session.ui_renderer:
                    active_session.ui_renderer.after(
                        50, update_callback
                    )  # 20 FPS
    ...
    def update(self):
        ...
        for event in self.event_queue:
            if (
                ...
                and event.session_id == self.active_session
            ):
        ...
        if processed_count > 0:
            self.session_logger.log_system(
                "DEBUG",
                "MULTI_GAME_DIRECTOR",
                f"Processed {processed_count} events for {
                    self.active_session}",
                {},
            )
    ...
    def schedule_event(
        ...
        if session_id is None:
            self.session_logger.log_system(
                "WARNING",
                "MULTI_GAME_DIRECTOR",
                "No active session for event scheduling",
    ...
    def _execute_event(self, event: ScheduledEvent):
        ...
        try:
            if event.event_type == "bot_action":
                ...
                self._handle_human_action_delay_complete(
                    event.data, active_session
                )
            elif event.event_type == "hands_review_auto_advance":
                self._handle_hands_review_auto_advance(
                    event.data, active_session
                )
            else:
                self.session_logger.log_system(
    ...
    def execute_player_action(
        ...
        if not active_session:
            self.session_logger.log_system(
                "ERROR",
                "MULTI_GAME_DIRECTOR",
                "No active session for player action",
                {},
            )
            return False

        # Route to the appropriate session's state machine
        return self._execute_player_action_for_session(
            player, action, amount, active_session
        )

    def _execute_player_action_for_session(
        self,
        player,
        action: ActionType,
        amount: float,
        ...
        try:
            success = session.state_machine.execute_action(
            ...
            self.session_logger.log_system(
                "ERROR",
                "MULTI_GAME_DIRECTOR",
                f"Error executing player action: {e}",
                {
                    "session_type": session.session_type,
                    "player": player.name,
                    "action": action.value,
    ...
    def schedule_next_bot_action(self):
        ...
        # HANDS REVIEW: No delays - all actions are immediate historical
        # replays
        ...
        if active_session.session_type == "practice":
            if hasattr(active_session.state_machine, "action_player_index"):
                ...
                if self._is_bot_turn_next(active_session.state_machine):
                    ...
                    self.schedule_event(
                        delay_ms, "bot_action", {"player_index": player_index}
                    )
    ...
    def _is_bot_turn_next(self, state_machine) -> bool:
        ...
        if (
            hasattr(state_machine, "action_player_index")
            and state_machine.action_player_index >= 0
            and state_machine.action_player_index
            < len(state_machine.game_state.players)
        ):

            current_player = state_machine.game_state.players[
                state_machine.action_player_index
            ]
            return not current_player.is_human

        return False

    def _handle_bot_action(
        self, data: Dict[str, Any], session: SessionComponents
    ):
    ...
    def _handle_human_action_delay_complete(
        self, data: Dict[str, Any], session: SessionComponents
    ):
    ...
    def _handle_hands_review_auto_advance(
        self, data: Dict[str, Any], session: SessionComponents
    ):

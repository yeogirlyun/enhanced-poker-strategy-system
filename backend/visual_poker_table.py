...
class VisualPokerTable:
    ...
    def __init__(self, parent_frame, strategy_data: StrategyData):
        ...
        self.player_radius = 180  # Increased radius for better spacing
        ...
        # Create canvas with better padding and centering
    ...
    def _create_controls(self, parent_frame):
        ...
        start_button = ttk.Button(
            control_frame, text="🎯 Start New Hand", command=self._start_visual_hand
        )
        ...
        for text, value in actions:
            ttk.Radiobutton(
                action_frame, text=text, variable=self.action_var, value=value
        ...
        submit_button = ttk.Button(
            action_frame, text="Submit", command=self._submit_visual_action
        )
        submit_button.pack(side=tk.LEFT, padx=5)

    def _draw_table(self):
        """Draw the poker table."""
        # Draw table outline with better margins
        margin = 60  # Increased margin for better spacing
        self.canvas.create_oval(
            margin,
            margin,
            self.table_width - margin,
            self.table_height - margin,
            fill="#0B6623",
            outline="#8B4513",
            width=3,
        )
        ...
        for i in range(0, self.table_width, 20):
            for j in range(0, self.table_height, 20):
                if (i - self.center_x) ** 2 + (j - self.center_y) ** 2 < (
                    self.player_radius - 40
                ) ** 2:
                    self.canvas.create_oval(
                        i, j, i + 2, j + 2, fill="#228B22", outline=""
                    )

        # Draw pot area - moved down to avoid card overlap
        self.canvas.create_oval(
            self.center_x - 25,
            self.center_y + 10,  # Moved down to avoid community cards
            self.center_x + 25,
            self.center_y + 60,
            fill="#FFD700",
            outline="#B8860B",
    ...
    def _draw_player(
        ...
        self.canvas.create_oval(
            x - 25, y - 25, x + 25, y + 25, fill=color, outline="black", width=2
        )

        # Player name
        name = "You" if player.is_human else f"P{player.name.split()[-1]}"
        self.canvas.create_text(
            x, y - 5, text=name, font=("Arial", 10, "bold"), fill="white"
        )

        # Position
        self.canvas.create_text(
            x, y + 10, text=player.position.value, font=("Arial", 8), fill="white"
        )

        # Stack size
        self.canvas.create_text(
            x, y + 35, text=f"${player.stack:.0f}", font=("Arial", 8), fill="white"
        )
    ...
    def _draw_hole_cards(self, player: Player, position: Tuple[int, int]):
        ...
        # Position cards below player with better spacing
        card_y = y + 60  # Increased distance from player

        for i, card in enumerate(player.cards):
            card_x = x - 30 + (i * 35)  # Better card spacing
    ...
    def _draw_community_cards(self):
        ...
        for i, card in enumerate(self.current_game_state.board):
            ...
            card_y = self.center_y - 40  # Moved up slightly
    ...
    def _draw_pot_info(self):
        ...
        # Pot amount - moved down to match pot area
        self.canvas.create_text(
            self.center_x,
            self.center_y + 35,  # Moved down to match pot area
        ...
        if self.current_game_state.current_bet > 0:
            self.canvas.create_text(
                self.center_x,
                self.center_y + 55,  # Moved down to match pot area
    ...
    def _show_action_feedback(self, action: Action, bet_size: float):
        ...
        feedback_text = tk.Text(
            feedback_window, font=("Arial", 12), wrap="word", padx=10, pady=10
        )
        ...
        # Close button
        ttk.Button(feedback_window, text="OK", command=feedback_window.destroy).pack(
            pady=10
        )
...
class VisualPokerTableGUI:
    ...
    def __init__(self, strategy_data: StrategyData):
        ...
        self.root.geometry("900x700")  # Larger window for better table display

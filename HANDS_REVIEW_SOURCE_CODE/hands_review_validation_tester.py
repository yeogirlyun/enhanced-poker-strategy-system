...
def main():
    ...
    print("=" * 50)
    print("🔍 DEBUG: Main function is executing!")
    
    # Parse command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default=os.environ.get('DATA_FILE', 'data/legendary_hands.json'))
    parser.add_argument('--limit', type=int, default=100)
    args = parser.parse_args()
    
    print(f"🔍 DEBUG: Parsed args.data = {args.data}")
    print(f"📁 Loading hands from: {args.data}")
    print(f"🎯 Target: Validate up to {args.limit} hands")
    print("=" * 60)
    
    # Create validator with the specified data path
    validator = HandsReviewValidator(args.data)
    
    # Run validation on up to the specified number of hands
    summary = validator.run_validation(max_hands=args.limit)
...
print("🔍 DEBUG: Module is being loaded/imported")

if __name__ == "__main__":
    print("🔍 DEBUG: Module is being run directly")
    main()

...
class ReusablePokerGameWidget(ttk.Frame, EventListener):
    ...
    # --- SIZING HELPERS (PERCENTAGE-BASED, ANCHORED TO CARD SIZE) ---
    def _get_num_players_safe(self) -> int:
        try:
    ...
    def _compute_metrics(self, width: int, height: int, num_players: int) -> dict:
        """Compute size metrics for all elements based on table size and player count."""
        s = min(width, height)
        if num_players <= 2:
            pct = 0.08
        elif num_players == 3:
        ...
        card_w = self._clamp(int(pct * s), 56, 110)
        card_h = self._clamp(int(1.40 * card_w), 78, 154)
        card_gap_x = max(6, int(0.007 * width))

        pod_inner_w = int(1.5 * (2 * card_w + card_gap_x))
        pod_inner_h = int(1.6 * card_h)

        pot_w = int(0.55 * card_h)
        pot_h = int(0.40 * card_h)
        ...
        return {
            "card_w": card_w,
            "card_h": card_h,
            "card_gap_x": card_gap_x,
            "pod_inner_w": pod_inner_w,
            "pod_inner_h": pod_inner_h,
            "pot_w": pot_w,
            "pot_h": pot_h,
            "bet_w": bet_w,
            "bet_h": bet_h,
    ...
    def reset_change_tracking(self):
        ...
        # Also clear any folded indicators and community cards
        try:
            # Remove folded labels and restore seat styling
            for i, seat in enumerate(self.player_seats):
                if seat and seat.get("frame"):
                    # Remove any child labels tagged as action indicators
                    for child in list(seat["frame"].winfo_children()):
                        try:
                            if getattr(child, "_action_indicator", False):
                                child.destroy()
                        except Exception:
                            pass
                    # Restore card widgets to unfolded/empty
                    for cw in seat.get("card_widgets", []):
                        try:
                            cw.set_card("", is_folded=False)
                        except Exception:
                            pass
            # Clear community cards immediately on reset
            if hasattr(self, "community_card_widgets"):
                for cw in self.community_card_widgets:
                    try:
                        cw.set_card("")
                    except Exception:
                        pass
    ...
    def _create_bet_display_for_player(self, player_index: int):
        ...
        # Create chip display for bets (scaled, borderless, compact)
        chip_display = ChipStackDisplay(self.canvas, amount=0.0, title="")
        try:
            m = self._compute_metrics(
                self.canvas.winfo_width(), self.canvas.winfo_height(), self._get_num_players_safe()
            )
            if hasattr(chip_display, "chip_canvas"):
                chip_display.chip_canvas.config(width=m["bet_w"], height=m["bet_h"])
        except Exception:
            pass
    ...
    def update_display(self, update_type: str, **kwargs):
        ...
        elif update_type == "hand_start":
            # Alias for immediate full refresh at the beginning of a hand
            # This ensures hole cards render right after Reset/Load
            self._update_from_fpsm_state()
    ...
    def _handle_round_complete(self, event: GameEvent):
        ...
        # snapshot from event (exclude folded players)
        snapshot = (
        ...
        if snapshot:
            ...
            for item in snapshot:
                ...
                # Skip animation for folded or inactive players
                is_folded = False
                try:
                    if hasattr(self, "state_machine") and self.state_machine:
                        gs_players = self.state_machine.get_game_info().get("players", [])
                        if 0 <= idx < len(gs_players):
                            is_folded = bool(gs_players[idx].get("has_folded", False))
                except Exception:
                    is_folded = False

                if idx >= 0 and amt > 0 and not is_folded:
                    self.after(
                        animation_delay,
    ...
    def _is_canvas_ready(self, min_w: int = 200, min_h: int = 200) -> bool:
        try:
            return (
                hasattr(self, "canvas")
                and self.canvas
                and self.canvas.winfo_width() >= min_w
                and self.canvas.winfo_height() >= min_h
            )
        except Exception:
            return False

    def _wait_then(self, func, attempts: int = 12, delay_ms: int = 90):
        if self._is_canvas_ready():
            try:
                func()
            except Exception as e:
                debug_log(f"wait_then error: {e}", "POT_DISPLAY")
            return
        if attempts <= 0:
            debug_log("Canvas never became ready; give up creating pot", "POT_DISPLAY")
            return
        try:
            self.after(delay_ms, lambda: self._wait_then(func, attempts - 1, delay_ms))
        except Exception:
            pass
    ...
    def update_pot_amount(self, amount):
        ...
        if hasattr(self, "pot_display") and self.pot_display:
            ...
            try:
                if hasattr(self, "pot_display_canvas_id"):
                    self.canvas.itemconfigure(self.pot_display_canvas_id, state="normal")
                    self.canvas.tag_raise(self.pot_display_canvas_id)
            except Exception:
                pass
        ...
        def _create_and_set():
            ...
            if hasattr(self, "pot_display") and self.pot_display:
                ...
                try:
                    ...
                    pass
        self._wait_then(_create_and_set)
        self.last_pot_amount = amount
    ...
    def _ensure_seats_created_and_update(self):
        ...
        if hasattr(self, "state_machine") and self.state_machine:
            # Support sessions that expose game_state via property
            try:
                expected_player_count = len(self.state_machine.game_state.players)
            except Exception:
                gi = self.state_machine.get_game_info()
                expected_player_count = len(gi.get("players", [])) if gi else None
    ...
    def _draw_community_card_area(self):
        ...
        # Compute metrics and enforce consistent card size with hole cards
        num_players = self._get_num_players_safe()
        m = self._compute_metrics(width, height, num_players)
        # Keep minimum gap to lowest seat by moving the board upward if needed
        player_positions = self.layout_manager.calculate_player_positions(width, height, num_players, widget=self)
        bottom_y = max(y for (_, y) in player_positions) if player_positions else int(height * 0.82)
        min_gap = int(height * 0.06)
        # approximate board bottom at community_y + 0.5*card_h
        desired_bottom = community_y + m["card_h"] // 2
        if bottom_y - desired_bottom < min_gap:
            community_y = bottom_y - min_gap - m["card_h"] // 2

        # Create community card frame
        community_frame = tk.Frame(self.canvas, bg=THEME["secondary_bg"], bd=0)

        # Create cards frame (remove title for more vertical space)
        ...
        self.community_card_widgets = []
        # Use the same card size as player hole cards (consistency)
        card_w = m["card_w"]
        card_h = m["card_h"]
        pad_x = max(2, m["card_gap_x"] // 2)
        for i in range(5):
            card_widget = CardWidget(cards_frame, width=card_w, height=card_h)
            card_widget.pack(side=tk.LEFT, padx=pad_x)
            self.community_card_widgets.append(card_widget)

        # Store the community frame
        self.community_frame = community_frame
        self.community_window_id = self.canvas.create_window(
    ...
    def _draw_pot_display(self):
        ...
        # Ensure pot is below community cards and above bottom seats
        try:
            num_players = len(getattr(self.state_machine, 'game_state', None).players)
        except Exception:
            num_players = 6
        player_positions = self.layout_manager.calculate_player_positions(width, height, num_players, widget=self)
        bottom_y = max(y for (_, y) in player_positions) if player_positions else height * 0.85
        # If pot overlaps community frame vertically, nudge it down
        if hasattr(self, 'community_frame') and self.community_frame:
            try:
                cxy = self.canvas.coords(getattr(self, 'community_window_id', None))
                if cxy:
                    _, c_y = cxy
                    min_gap = int(height * 0.035)
                    if pot_y - c_y < min_gap:
                        pot_y = c_y + min_gap
            except Exception:
                pass
        # Keep pot above bottom players
        pot_y = min(pot_y, bottom_y - int(height * 0.06))

        # Create modern pot chip display (scale to table size)
        # Size pot widget from metrics
        self.pot_display = ChipStackDisplay(self.canvas, amount=0.0, title="Pot")
        try:
            m = self._compute_metrics(width, height, self._get_num_players_safe())
            if hasattr(self.pot_display, "chip_canvas"):
                self.pot_display.chip_canvas.config(width=m["pot_w"], height=m["pot_h"])
        except Exception:
            pass
        ...
        try:
            ...
            # As a last resort, schedule a deferred creation once canvas settles
            def _deferred():
                if not hasattr(self, 'pot_display_canvas_id'):
                    self.pot_display_canvas_id = self.canvas.create_window(
                        self.canvas.winfo_width()//2,
                        int(self.canvas.winfo_height()*0.58),
                        window=self.pot_display,
                        anchor="center",
                    )
            try:
                self.after(120, _deferred)
            except Exception:
                pass
    ...
    def _on_resize(self, event=None):
        ...
        if event and event.width > 1 and event.height > 1:
            ...
            # Reposition the toolbar at top-right
            try:
                if hasattr(self, "toolbar_frame") and self.toolbar_frame:
                    pad_x = 16
                    pad_y = 12
                    self.canvas.coords(
                        getattr(self, "toolbar_canvas_id", None),
                        self.canvas.winfo_width() - pad_x,
                        pad_y,
                    )
            except Exception:
                pass
    ...
    def _update_from_fpsm_state(self):
        ...
        game_info = self.state_machine.get_game_info()
        # Force reveal in review sessions if FPSM didn't propagate yet
        try:
            # If any player cards are still placeholders but FPSM mode is review,
            # refresh once more to fetch actual hole cards.
            if getattr(getattr(self, 'state_machine', None), 'fpsm', None):
                fpsm = self.state_machine.fpsm
                if getattr(fpsm, 'mode', '') == 'review':
                    players = game_info.get('players', [])
                    needs_reveal = any(p.get('cards') == ['**', '**'] for p in players)
                    if needs_reveal:
                        # Replace with cards directly from FPSM game_state
                        for i, p in enumerate(players):
                            try:
                                actual = fpsm.game_state.players[i].cards
                                p['cards'] = actual if actual else p.get('cards', [])
                            except Exception:
                                pass
        except Exception:
            pass
    ...
    def _setup_ui(self):
        ...
        # Allow external container to set height; otherwise, ensure a minimum
        try:
            self.canvas.configure(height=max(400, self.winfo_height()))
        except Exception:
            pass

        # Restore top-right control toolbar (zoom/reset)
        try:
            self.toolbar_frame = tk.Frame(self.canvas, bg=THEME["secondary_bg"], bd=0)
            btn_font = ("Arial", 9, "bold")
            zoom_in = tk.Button(
                self.toolbar_frame,
                text="＋",
                font=btn_font,
                command=self.increase_table_size,
                relief="flat",
                bg=THEME["secondary_bg"],
                fg="#eaecef",
                bd=0,
            ...
            zoom_out = tk.Button(
                ...
                text="－",
                font=btn_font,
                command=self.decrease_table_size,
                relief="flat",
                bg=THEME["secondary_bg"],
                fg="#eaecef",
                bd=0,
            ...
            reset_btn = tk.Button(
                ...
                text="⟳",
                font=btn_font,
                command=self._force_complete_redraw,
                relief="flat",
                bg=THEME["secondary_bg"],
                fg="#eaecef",
                bd=0,
            ...
            pad_x = 16
            pad_y = 12
            self.toolbar_canvas_id = self.canvas.create_window(
                self.canvas.winfo_width() - pad_x,
                pad_y,
                window=self.toolbar_frame,
                anchor="ne",
            )
        except Exception:
            pass
    ...
    def _draw_table(self):
        ...
        # Draw player seats (slightly more compact when height is limited)
        self._draw_player_seats()
    ...
    class LayoutManager:
        ...
        def calculate_player_positions(self, width, height, num_players, widget=None):
            ...
            # Special heads-up layout: left/right to avoid board overlap
            if num_players == 2:
                radius_x = width * 0.30
                # Lift top seat slightly and drop bottom slightly to open center
                up_down = height * 0.03
                return [
                    (center_x - radius_x, center_y + up_down),
                    (center_x + radius_x, center_y - up_down),
                ]

            # More compact radius for better space utilization
            base_radius_x = width * 0.32 if height < 650 else width * 0.35
            base_radius_y = height * 0.24 if height < 650 else height * 0.28

            # Adjust for different player counts
            if num_players <= 4:
                radius_x = base_radius_x * 0.85
                radius_y = base_radius_y * 0.80  # pull top/bottom away from center
            elif num_players <= 6:
                radius_x = base_radius_x
                radius_y = base_radius_y * 0.90
            else:
            ...
            for i in range(num_players):
                ...
                # Nudge pure top/bottom seats further from center to create board/pot space
                deg = (angle + math.pi / 2) * 180 / math.pi  # 0 deg at top
                if abs(deg) < 12 or abs(abs(deg) - 180) < 12:
                    y += height * 0.03 if deg > 0 else -height * 0.03
        ...
        def calculate_community_card_position(self, width, height):
            ...
            center_x, center_y = width / 2, height / 2
            # Place slightly lower to open space near top seat
            community_y = center_y - max(40, height * 0.075)
